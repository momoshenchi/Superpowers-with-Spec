# Superpowers Parallel Delta Remediation Plan
...
4. **Document interim manual mitigation.**
   - Update `superpowers/AGENTS.md` and docs so contributors know to rerun `superpowers change sync` (see Phase 1) whenever another change lands.
...
1. **Introduce `superpowers change sync <id>` (or `rebase`).**
...
2. **Enrich validator messages.**
   - `superpowers validate` should flag unresolved conflict markers or fingerprint mismatches so errors appear early in the workflow.
...
3. **Integrate with Git directly.**
   - Offer optional `superpowers branch` scaffolding that aligns spec changes with Git branches, letting teams leverage Git’s conflict editor for the markdown IR.

_Outcome:_ Superpowers graduates from replace-based updates to a resilient, intent-preserving spec management platform.
...
1. Prototype fingerprint capture during `superpowers change validate` and block archive on mismatches.
2. Ship `superpowers change sync` with line-based diff3 merging and conflict markers.
