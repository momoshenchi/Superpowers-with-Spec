...
describe('UpdateCommand', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = path.join(os.tmpdir(), `superpowers-test-${randomUUID()}`);
  ...
  describe('basic validation', () => {
    it('should throw error if superpowers directory does not exist', async () => {
      // Remove superpowers directory
      await fs.rm(path.join(testDir, 'superpowers'), {
        recursive: true,
        force: true,
      });

      await expect(updateCommand.execute(testDir)).rejects.toThrow(
        "No Superpowers directory found. Run 'superpowers init' first."
      );
    });
...
`;
      ...
      expect(updatedSkill).toContain('name: superpowers-explore');
    ...
    it('should update core profile skill files when tool is configured', async () => {
      ...
      await fs.mkdir(path.join(skillsDir, 'superpowers-explore'), {
        recursive: true,
      });
      await fs.writeFile(
  ...
  describe('command updates', () => {
    it('should update sp commands for configured Claude tool', async () => {
      ...
      // Check sp command files were created
    ...
    it('should update core profile sp commands when tool is configured', async () => {
  ...
  describe('multi-tool support', () => {
    it('should update multiple configured tools', async () => {
      ...
      await fs.mkdir(path.join(claudeSkillsDir, 'superpowers-explore'), {
        recursive: true,
      });
      await fs.writeFile(
      ...
      await fs.mkdir(path.join(cursorSkillsDir, 'superpowers-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(cursorSkillsDir, 'superpowers-explore', 'SKILL.md'),
        'old'
      );
      ...
      expect(claudeSkill).toContain('name: superpowers-explore');
    ...
    it('should update Qwen tool with correct command format', async () => {
      ...
      await fs.mkdir(path.join(qwenSkillsDir, 'superpowers-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(qwenSkillsDir, 'superpowers-explore', 'SKILL.md'),
        'old'
      );

      await updateCommand.execute(testDir);

      // Check Qwen command format (TOML) - Qwen uses flat path structure: sp-<id>.toml
    ...
    it('should update Windsurf tool with correct command format', async () => {
      ...
      await fs.mkdir(path.join(windsurfSkillsDir, 'superpowers-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(windsurfSkillsDir, 'superpowers-explore', 'SKILL.md'),
        'old'
      );
  ...
  describe('skill content validation', () => {
    ...
    it('should include proper instructions in skill files', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'superpowers-apply-change'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(skillsDir, 'superpowers-apply-change', 'SKILL.md'),
        'old'
      );
...
`
      ...
      const cursorSkillDir = path.join(testDir, '.cursor', 'skills', 'superpowers-explore');
...
`
  ...
  describe('version tracking', () => {
    ...
    it('should only update tools that need updating', async () => {
      ...
      const claudeSkillFile = path.join(testDir, '.claude', 'skills', 'superpowers-explore', 'SKILL.md');
  ...
  describe('legacy cleanup', () => {
    it('should detect and auto-cleanup legacy files with --force flag', async () => {
      ...
      // Create legacy CLAUDE.md with Superpowers markers
      const legacyContent = `${SUPERPOWERS_MARKERS.start}
...
`;
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Upgrading to the new Superpowers')
      );

      // Should show marker removal message (config files are never deleted, only have markers removed)
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Removed Superpowers markers from CLAUDE.md')
      );
...
`;
    ...
    it('should cleanup legacy slash command directories with --force', async () => {
      ...
      const legacyCommandDir = path.join(testDir, '.claude', 'commands', 'superpowers');
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Removed .claude/commands/superpowers/')
      );
    ...
    it('should cleanup legacy superpowers/AGENTS.md with --force', async () => {
      // Set up a configured tool
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'superpowers-explore'), {
        recursive: true,
      ...
      // Create legacy superpowers/AGENTS.md
      await fs.writeFile(
        path.join(testDir, 'superpowers', 'AGENTS.md'),
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Removed superpowers/AGENTS.md')
      );
    ...
    it('should not show legacy cleanup messages when no legacy files exist', async () => {
      ...
      const hasLegacyMessage = calls.some(call =>
        call.includes('Upgrading to the new Superpowers')
      );
    ...
    it('should remove Superpowers marker block from mixed content files', async () => {
      // Set up a configured tool
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'superpowers-explore'), {
        recursive: true,
      ...
      // Create CLAUDE.md with mixed content (user content + Superpowers markers)
...
`;
      ...
      expect(updatedContent).not.toContain(SUPERPOWERS_MARKERS.start);
      expect(updatedContent).not.toContain(SUPERPOWERS_MARKERS.end);
  ...
  describe('legacy tool upgrade', () => {
    it('should upgrade legacy tools to new skills with --force', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('/sp:new')
      );
    ...
    it('should upgrade multiple legacy tools with --force', async () => {
      ...
      await fs.writeFile(
        path.join(testDir, '.claude', 'commands', 'superpowers', 'proposal.md'),
        'content'
      );

      await fs.mkdir(path.join(testDir, '.cursor', 'commands'), { recursive: true });
      await fs.writeFile(
        path.join(testDir, '.cursor', 'commands', 'superpowers-proposal.md'),
        'content'
      );
      ...
      const claudeSkillFile = path.join(testDir, '.claude', 'skills', 'superpowers-explore', 'SKILL.md');
      const cursorSkillFile = path.join(testDir, '.cursor', 'skills', 'superpowers-explore', 'SKILL.md');
    ...
    it('should upgrade only unconfigured legacy tools when mixed', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'superpowers-explore'), { recursive: true });
      await fs.writeFile(
        path.join(claudeSkillsDir, 'superpowers-explore', 'SKILL.md'),
        'existing skill'
      );
    ...
    it('should create only effective profile skills when upgrading legacy tools', async () => {
      ...
      const nonCoreSkill = path.join(skillsDir, 'superpowers-new-change', 'SKILL.md');
    ...
    it('should create commands when upgrading legacy tools', async () => {
      ...
      // New sp commands should be created
      const commandsDir = path.join(testDir, '.claude', 'commands', 'sp');
      const exploreCmd = path.join(commandsDir, 'explore.md');
      const exists = await FileSystemUtils.fileExists(exploreCmd);
      expect(exists).toBe(true);
    ...
    it('should not inject non-profile workflows when upgrading legacy tools', async () => {
      ...
      expect(await FileSystemUtils.fileExists(
        path.join(skillsDir, 'superpowers-propose', 'SKILL.md')
      )).toBe(false);
...
`
    ...
    it('should remove workflows outside profile during update sync', async () => {
      ...
      await fs.mkdir(path.join(skillsDir, 'superpowers-new-change'), { recursive: true });
      await fs.writeFile(path.join(skillsDir, 'superpowers-new-change', 'SKILL.md'), 'old');
      const extraCommandFile = path.join(testDir, '.claude', 'commands', 'sp', 'new.md');
  ...
  describe('new tool detection', () => {
    it('should detect new tool directories not currently configured', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'superpowers-explore'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'superpowers-explore', 'SKILL.md'), 'old');
      ...
      const hasNewToolMessage = calls.some(call =>
        call.includes("Detected new tool: Cursor. Run 'superpowers init' to add it.")
      );
    ...
    it('should consolidate multiple new tools into one message', async () => {
      ...
      expect(consolidatedCalls[0]).toContain("Run 'superpowers init' to add them.");
  ...
  describe('scanInstalledWorkflows', () => {
    it('should detect installed workflows across tools', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'superpowers-explore'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'superpowers-explore', 'SKILL.md'), 'content');
      await fs.mkdir(path.join(claudeSkillsDir, 'superpowers-apply-change'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'superpowers-apply-change', 'SKILL.md'), 'content');
    ...
    it('should return union of workflows across multiple tools', async () => {
      ...
      // Cursor has apply
      const cursorSkillsDir = path.join(testDir, '.cursor', 'skills');
      await fs.mkdir(path.join(cursorSkillsDir, 'superpowers-apply-change'), { recursive: true });
      await fs.writeFile(path.join(cursorSkillsDir, 'superpowers-apply-change', 'SKILL.md'), 'content');
