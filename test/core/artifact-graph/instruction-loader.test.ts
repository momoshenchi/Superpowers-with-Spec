...
describe('instruction-loader', () => {
  ...
  describe('loadChangeContext', () => {
    ...
    beforeEach(() => {
      tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'superpowers-test-'));
    });
    ...
    it('should auto-detect schema from .superpowers.yaml metadata', () => {
      // Create change directory with metadata file
      const changeDir = path.join(tempDir, 'superpowers', 'changes', 'my-change');
      fs.mkdirSync(changeDir, { recursive: true });
      fs.writeFileSync(path.join(changeDir, '.superpowers.yaml'), 'schema: spec-driven\ncreated: "2025-01-05"\n');
  ...
  describe('generateInstructions', () => {
    ...
    it('should include execution-plan template content and task dependency context', () => {
      const changeDir = path.join(tempDir, 'superpowers', 'changes', 'my-change');
      fs.mkdirSync(changeDir, { recursive: true });
      fs.writeFileSync(path.join(changeDir, 'tasks.md'), '# 1. Tests\n\n- [ ] 1.1 Add tests');
      ...
      expect(instructions.template).toContain('Dispatch Coordination');
      ...
      expect(instructions.template).toContain('Implementation Notes');
      expect(instructions.template).toContain('non-normative');
      expect(instructions.template).not.toContain('Apply Handoff and Final Quality Gates');
    ...
    it('should include test-plan template content and execution-plan dependency context', () => {
      ...
      expect(instructions.template).toContain('A **test object** is one coverage unit');
      expect(instructions.template).toContain('not a folder, not a file list, and not a Scenario');
      expect(instructions.template).toContain('Spec Scenarios to import');
      expect(instructions.template).toContain('## Design Contract And Invariant Coverage');
      expect(instructions.template).toContain('| Object | Requirement | Spec Scenario | D1 Case ID | Related Case IDs | Notes |');
      expect(instructions.template).toContain('This table is not a second case list');
      expect(instructions.template).not.toContain('| Object | Requirement / Spec Scenario | Form | Status | Notes |');
      expect(instructions.template).toContain('| ID | Object | Source | Scenario Type | Steps | Expected | Form | Status | Evidence |');
      expect(instructions.template).toContain('imported:');
      expect(instructions.template).toContain('A thin spec does not cap coverage');
      expect(instructions.template).toContain('spec omitted');
      expect(instructions.template).toContain('wrong implementation could still pass');
      expect(instructions.template).toContain("Every path or symbol in the object's Diff Anchor");
      expect(instructions.template).toContain('every planned `unit` case across D1–D6');
      expect(instructions.template).toContain('not after D2 alone');
      expect(instructions.template).toContain('from `full-qa-test` Step 7');
      expect(instructions.template).toContain('mirroring the `full-qa-test` Step 8 self-check');
      expect(instructions.template).not.toContain('Step 2b');
    ...
    describe('project config integration', () => {
      it('should return context as separate field for all artifacts', () => {
        ...
        const configDir = path.join(tempDir, 'superpowers');
...
`
      ...
      it('should deduplicate validation warnings within session', () => {
        ...
        const freshTempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'superpowers-test-'));

        try {
          // Create project config with a uniquely named invalid artifact ID
          const configDir = path.join(freshTempDir, 'superpowers');
          fs.mkdirSync(configDir, { recursive: true });
          fs.writeFileSync(
            path.join(configDir, 'config.yaml'),
            `schema: spec-driven
