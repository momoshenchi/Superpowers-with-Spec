...
describe('ArchiveCommand', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = path.join(os.tmpdir(), `superpowers-archive-test-${Date.now()}`);
    await fs.mkdir(tempDir, { recursive: true });
    
    // Change to temp directory
    process.chdir(tempDir);
    
    // Create Superpowers structure
    const superpowersDir = path.join(tempDir, 'superpowers');
    await fs.mkdir(path.join(superpowersDir, 'changes'), { recursive: true });
    await fs.mkdir(path.join(superpowersDir, 'specs'), { recursive: true });
    await fs.mkdir(path.join(superpowersDir, 'changes', 'archive'), { recursive: true });
...
Then expected result happens`;
      ...
      const mainSpecPath = path.join(tempDir, 'superpowers', 'specs', 'test-capability', 'spec.md');
...
### Requirement: Thumbnail Field`;
      ...
      const mainSpecPath = path.join(tempDir, 'superpowers', 'specs', 'gift-card', 'spec.md');
      const updatedContent = await fs.readFile(mainSpecPath, 'utf-8');
...
Modified content.`;
      ...
      const mainSpecPath = path.join(tempDir, 'superpowers', 'specs', 'new-capability', 'spec.md');
...
- TO: \`### Requirement: New Name\``;
      ...
      // Verify spec was NOT created
      const mainSpecPath = path.join(tempDir, 'superpowers', 'specs', 'another-capability', 'spec.md');
      await expect(fs.access(mainSpecPath)).rejects.toThrow();
    ...
    it('should throw error if archive already exists', async () => {
      ...
      const archivePath = path.join(tempDir, 'superpowers', 'changes', 'archive', `${date}-${changeName}`);
...
Then expected result happens`;
    ...
    it('should support header trim-only normalization for matching', async () => {
      ...
      const mainSpecDir = path.join(tempDir, 'superpowers', 'specs', 'alpha');
...
Updated details.`;
    ...
    it('should apply operations in order: RENAMED → REMOVED → MODIFIED → ADDED', async () => {
      ...
      const mainSpecDir = path.join(tempDir, 'superpowers', 'specs', 'beta');
      await fs.mkdir(mainSpecDir, { recursive: true });
...
content D`;
    ...
    it('should abort with error when MODIFIED/REMOVED reference non-existent requirements', async () => {
      ...
      const mainSpecDir = path.join(tempDir, 'superpowers', 'specs', 'gamma');
      await fs.mkdir(mainSpecDir, { recursive: true });
...
### Requirement: Another Missing`;
    ...
    it('should require MODIFIED to reference the NEW header when a rename exists (error format)', async () => {
      ...
      // Main spec with Old
      const mainSpecDir = path.join(tempDir, 'superpowers', 'specs', 'delta');
      await fs.mkdir(mainSpecDir, { recursive: true });
...
new body`;
    ...
    it('should process multiple specs atomically (any failure aborts all)', async () => {
      ...
      // Existing main specs
      const epsilonMain = path.join(tempDir, 'superpowers', 'specs', 'epsilon', 'spec.md');
...
e1`);
      ...
      const zetaMain = path.join(tempDir, 'superpowers', 'specs', 'zeta', 'spec.md');
...
### Requirement: Missing`);
    ...
    it('should display aggregated totals across multiple specs', async () => {
      ...
      // Existing main specs
      const omegaMain = path.join(tempDir, 'superpowers', 'specs', 'omega', 'spec.md');
      await fs.mkdir(path.dirname(omegaMain), { recursive: true });
      await fs.writeFile(omegaMain, `# omega Specification\n\n## Purpose\nOmega purpose.\n\n## Requirements\n\n### Requirement: O1\no1`);

      const psiMain = path.join(tempDir, 'superpowers', 'specs', 'psi', 'spec.md');
  ...
  describe('error handling', () => {
    it('should throw error when superpowers directory does not exist', async () => {
      // Remove superpowers directory
      await fs.rm(path.join(tempDir, 'superpowers'), { recursive: true });
      
      await expect(
        archiveCommand.execute('any-change', { yes: true })
      ).rejects.toThrow("No Superpowers changes directory found. Run 'superpowers init' first.");
    });
  });
  ...
  describe('interactive mode', () => {
    it('should use select prompt for change selection', async () => {
      ...
      await fs.mkdir(path.join(tempDir, 'superpowers', 'changes', change1), { recursive: true });
      await fs.mkdir(path.join(tempDir, 'superpowers', 'changes', change2), { recursive: true });
