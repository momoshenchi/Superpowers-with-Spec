...
describe('skill-generation', () => {
  describe('getSkillTemplates', () => {
    ...
    it('should include all expected skills', () => {
      ...
      expect(dirNames).toContain('superpowers-explore');
      expect(dirNames).toContain('superpowers-new-change');
      expect(dirNames).toContain('superpowers-continue-change');
      expect(dirNames).toContain('superpowers-apply-change');
      expect(dirNames).toContain('superpowers-ff-change');
      expect(dirNames).toContain('superpowers-sync-specs');
      expect(dirNames).toContain('superpowers-archive-change');
      expect(dirNames).toContain('superpowers-bulk-archive-change');
      ...
      expect(dirNames).toContain('superpowers-change-review');
    });
  ...
  describe('generateSkillContent', () => {
    ...
    it('should use default values for optional fields', () => {
      ...
      expect(content).toContain('license: MIT');
      expect(content).toContain('compatibility: Requires superpowers CLI.');
      expect(content).toContain('author: superpowers');
    ...
    it('should apply transformInstructions callback when provided', () => {
      const template = {
        ...
        instructions: 'Use /sp:new to start and /sp:apply to implement.',
      };

      const transformer = (text: string) => text.replace(/\/sp:/g, '/sp-');
      const content = generateSkillContent(template, '0.23.0', transformer);

      expect(content).toContain('/sp-new');
      expect(content).toContain('/sp-apply');
      expect(content).not.toContain('/sp:new');
      expect(content).not.toContain('/sp:apply');
    });
    ...
    it('should not transform instructions when callback is undefined', () => {
      const template = {
        ...
        instructions: 'Use /sp:new to start.',
      };
