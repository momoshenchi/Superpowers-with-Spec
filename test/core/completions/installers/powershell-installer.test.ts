...
describe('PowerShellInstaller', () => {
  ...
  beforeEach(async () => {
    testHomeDir = path.join(os.tmpdir(), `superpowers-powershell-test-${randomUUID()}`);
  ...
  describe('getInstallationPath', () => {
    it('should return path relative to profile directory', () => {
      ...
      expect(result).toBe(path.join(testHomeDir, '.config', 'powershell', 'SuperpowersCompletion.ps1'));
    });

    it('should work with custom PROFILE environment variable', () => {
      process.env.PROFILE = path.join(testHomeDir, 'custom', 'profile.ps1');
      const result = installer.getInstallationPath();
      expect(result).toBe(path.join(testHomeDir, 'custom', 'SuperpowersCompletion.ps1'));
    });
    ...
    it('should return Windows path when on Windows platform', () => {
      ...
      const result = installer.getInstallationPath();
      expect(result).toBe(path.join(testHomeDir, 'Documents', 'PowerShell', 'SuperpowersCompletion.ps1'));
    });
  });
  ...
  describe('configureProfile', () => {
    const mockScriptPath = '/path/to/SuperpowersCompletion.ps1';

    // Note: SUPERPOWERS_NO_AUTO_CONFIG check is now handled in the install() method,
    ...
    it.skipIf(process.platform === 'win32')('should skip configuration when script line already exists', async () => {
      ...
      const initialContent = [
        '# SUPERPOWERS:START - Superpowers completion (managed block, do not edit manually)',
        `. "${mockScriptPath}"`,
        '# SUPERPOWERS:END',
        '',
        '# My custom config',
  ...
  describe('removeProfileConfig', () => {
    ...
    it('should remove content between markers', async () => {
      ...
      expect(content).not.toContain('# Superpowers completions');
  ...
  describe('install', () => {
    const mockCompletionScript = `# PowerShell completion script for Superpowers
$superpowersCompleter = {
    param($wordToComplete, $commandAst, $cursorPosition)
    # Completion logic here
}
Register-ArgumentCompleter -CommandName superpowers -ScriptBlock $superpowersCompleter
`;
    ...
    it('should install completion script for the first time', async () => {
      ...
      expect(result.installedPath).toContain('SuperpowersCompletion.ps1');
    ...
    // Note: SUPERPOWERS_NO_AUTO_CONFIG support was removed from PowerShell installer
    ...
    it('should handle installation with paths containing spaces', async () => {
      const spacedHomeDir = path.join(os.tmpdir(), `superpowers powershell test ${randomUUID()}`);
      ...
      expect(result.success).toBe(true);
      expect(result.installedPath).toContain('superpowers powershell test');
...
`;
    ...
    it('should handle both script and config removal', async () => {
      ...
      expect(profileContent).toContain('# SUPERPOWERS:START');
      ...
      expect(profileContentAfter).not.toContain('# SUPERPOWERS:START');
    });
