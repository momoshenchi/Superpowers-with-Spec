...
describe('PowerShellGenerator', () => {
	...
	describe('generate', () => {
		it('should generate valid PowerShell completion script with header', () => {
			...
			expect(script).toContain('# PowerShell completion script for Superpowers CLI');
			expect(script).toContain('$superpowersCompleter = {');
			expect(script).toContain('Register-ArgumentCompleter');
		});

		it('should register argument completer for superpowers command', () => {
			const commands: CommandDefinition[] = [
				{
					name: 'init',
					description: 'Initialize Superpowers',
			...
			expect(script).toContain('Register-ArgumentCompleter -CommandName superpowers');
			expect(script).toContain('-ScriptBlock $superpowersCompleter');
		});
		...
		it('should handle positional arguments for change-id', () => {
			...
			expect(script).toContain('Get-SuperpowersChanges');
		});
		...
		it('should handle positional arguments for spec-id', () => {
			...
			expect(script).toContain('Get-SuperpowersSpecs');
		});
		...
		it('should generate dynamic completion helper for changes', () => {
			...
			expect(script).toContain('function Get-SuperpowersChanges');
			expect(script).toContain('superpowers __complete changes 2>$null');
		...
		it('should generate dynamic completion helper for specs', () => {
			...
			expect(script).toContain('function Get-SuperpowersSpecs');
			expect(script).toContain('superpowers __complete specs 2>$null');
		});
