...
import {
  ...
  hasSuperpowersMarkers,
  isOnlySuperpowersContent,
  removeMarkerBlock,
...
import { SUPERPOWERS_MARKERS } from '../../src/core/config.js';
...
describe('legacy-cleanup', () => {
  ...
  beforeEach(async () => {
    testDir = path.join(os.tmpdir(), `superpowers-legacy-test-${randomUUID()}`);
  ...
  describe('hasSuperpowersMarkers', () => {
    it('should return true when both markers are present', () => {
      const content = `Some content
${SUPERPOWERS_MARKERS.start}
Superpowers content
${SUPERPOWERS_MARKERS.end}
More content`;
      expect(hasSuperpowersMarkers(content)).toBe(true);
    });
...
Superpowers content
${SUPERPOWERS_MARKERS.end}`;
      expect(hasSuperpowersMarkers(content)).toBe(false);
    });

    it('should return false when end marker is missing', () => {
      const content = `${SUPERPOWERS_MARKERS.start}
Superpowers content
Some content`;
      expect(hasSuperpowersMarkers(content)).toBe(false);
    });
  ...
  describe('isOnlySuperpowersContent', () => {
    it('should return true when content is only markers and whitespace outside', () => {
      const content = `${SUPERPOWERS_MARKERS.start}
Superpowers content here
${SUPERPOWERS_MARKERS.end}`;
      expect(isOnlySuperpowersContent(content)).toBe(true);
    });
...
Superpowers content
${SUPERPOWERS_MARKERS.end}`;
      expect(isOnlySuperpowersContent(content)).toBe(false);
    });
...
User content here`;
    ...
    it('should return false when end marker comes before start marker', () => {
      const content = `${SUPERPOWERS_MARKERS.end}
Content
${SUPERPOWERS_MARKERS.start}`;
      expect(isOnlySuperpowersContent(content)).toBe(false);
    });
...
${SUPERPOWERS_MARKERS.end}`;
      ...
      expect(result).not.toContain(SUPERPOWERS_MARKERS.start);
      expect(result).not.toContain(SUPERPOWERS_MARKERS.end);
    });
...
${SUPERPOWERS_MARKERS.start}`;
      const result = removeMarkerBlock(content);
      expect(result).toContain(SUPERPOWERS_MARKERS.end);
      expect(result).toContain(SUPERPOWERS_MARKERS.start);
    });

    it('should ignore inline mentions of markers and only remove actual block', () => {
      const content = `Intro referencing ${SUPERPOWERS_MARKERS.start} and ${SUPERPOWERS_MARKERS.end} inline.
...
After content`;
  ...
  describe('detectLegacyConfigFiles', () => {
    it('should detect CLAUDE.md with Superpowers markers and put in update list', async () => {
      const claudePath = path.join(testDir, 'CLAUDE.md');
      await fs.writeFile(claudePath, `${SUPERPOWERS_MARKERS.start}
Superpowers content
${SUPERPOWERS_MARKERS.end}`);
...
${SUPERPOWERS_MARKERS.end}`);
    ...
    it('should not detect files without Superpowers markers', async () => {
      const claudePath = path.join(testDir, 'CLAUDE.md');
    ...
    it('should detect multiple config files', async () => {
      ...
      await fs.writeFile(path.join(testDir, 'CLAUDE.md'), `${SUPERPOWERS_MARKERS.start}\nContent\n${SUPERPOWERS_MARKERS.end}`);
      await fs.writeFile(path.join(testDir, 'CLINE.md'), `${SUPERPOWERS_MARKERS.start}\nContent\n${SUPERPOWERS_MARKERS.end}`);
      await fs.writeFile(path.join(testDir, 'QODER.md'), `${SUPERPOWERS_MARKERS.start}\nContent\n${SUPERPOWERS_MARKERS.end}`);
  ...
  describe('detectLegacySlashCommands', () => {
    it('should detect legacy Claude slash command directory', async () => {
      const dirPath = path.join(testDir, '.claude', 'commands', 'superpowers');
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, 'proposal.md'), 'content');

      const result = await detectLegacySlashCommands(testDir);
      expect(result.directories).toContain('.claude/commands/superpowers');
    });

    it('should detect legacy Cursor slash command files', async () => {
      const dirPath = path.join(testDir, '.cursor', 'commands');
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, 'superpowers-proposal.md'), 'content');
      await fs.writeFile(path.join(dirPath, 'superpowers-apply.md'), 'content');

      const result = await detectLegacySlashCommands(testDir);
      expect(result.files).toContain('.cursor/commands/superpowers-proposal.md');
      expect(result.files).toContain('.cursor/commands/superpowers-apply.md');
    });

    it('should detect legacy Windsurf workflow files', async () => {
      const dirPath = path.join(testDir, '.windsurf', 'workflows');
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, 'superpowers-archive.md'), 'content');

      const result = await detectLegacySlashCommands(testDir);
      expect(result.files).toContain('.windsurf/workflows/superpowers-archive.md');
    });

    it('should detect multiple tool directories and files', async () => {
      // Create directory-based
      await fs.mkdir(path.join(testDir, '.claude', 'commands', 'superpowers'), { recursive: true });
      await fs.mkdir(path.join(testDir, '.qoder', 'commands', 'superpowers'), { recursive: true });

      // Create file-based
      await fs.mkdir(path.join(testDir, '.cursor', 'commands'), { recursive: true });
      await fs.writeFile(path.join(testDir, '.cursor', 'commands', 'superpowers-proposal.md'), 'content');

      const result = await detectLegacySlashCommands(testDir);
      expect(result.directories).toContain('.claude/commands/superpowers');
      expect(result.directories).toContain('.qoder/commands/superpowers');
      expect(result.files).toContain('.cursor/commands/superpowers-proposal.md');
    });

    it('should not detect non-superpowers files', async () => {
      const dirPath = path.join(testDir, '.cursor', 'commands');
      await fs.mkdir(dirPath, { recursive: true });
    ...
    it('should detect TOML-based slash commands for Qwen', async () => {
      ...
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, 'superpowers-proposal.toml'), 'content');

      const result = await detectLegacySlashCommands(testDir);
      expect(result.files).toContain('.qwen/commands/superpowers-proposal.toml');
    });

    it('should detect Continue prompt files', async () => {
      const dirPath = path.join(testDir, '.continue', 'prompts');
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, 'superpowers-apply.prompt'), 'content');

      const result = await detectLegacySlashCommands(testDir);
      expect(result.files).toContain('.continue/prompts/superpowers-apply.prompt');
    });

    it('should detect legacy OpenCode sp-* command files', async () => {
      const dirPath = path.join(testDir, '.opencode', 'command');
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, 'sp-propose.md'), 'content');

      const result = await detectLegacySlashCommands(testDir);
      expect(result.files).toContain('.opencode/command/sp-propose.md');
    });

    it('should detect legacy OpenCode superpowers-* command files', async () => {
      const dirPath = path.join(testDir, '.opencode', 'command');
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, 'superpowers-new.md'), 'content');

      const result = await detectLegacySlashCommands(testDir);
      expect(result.files).toContain('.opencode/command/superpowers-new.md');
    });

    it('should detect both sp-* and superpowers-* OpenCode command files', async () => {
      const dirPath = path.join(testDir, '.opencode', 'command');
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, 'sp-propose.md'), 'content');
      await fs.writeFile(path.join(dirPath, 'superpowers-new.md'), 'content');
  ...
  describe('detectLegacyStructureFiles', () => {
    it('should detect superpowers/AGENTS.md', async () => {
      const agentsPath = path.join(testDir, 'superpowers', 'AGENTS.md');
    ...
    it('should detect superpowers/project.md', async () => {
      const projectPath = path.join(testDir, 'superpowers', 'project.md');
    ...
    it('should detect root AGENTS.md with Superpowers markers', async () => {
      const agentsPath = path.join(testDir, 'AGENTS.md');
      await fs.writeFile(agentsPath, `${SUPERPOWERS_MARKERS.start}
Superpowers content
${SUPERPOWERS_MARKERS.end}`);
  ...
  describe('detectLegacyArtifacts', () => {
    ...
    it('should return hasLegacyArtifacts: true when slash commands are found', async () => {
      ...
      const result = await detectLegacyArtifacts(testDir);
      expect(result.hasLegacyArtifacts).toBe(true);
      expect(result.slashCommandDirs).toContain('.claude/commands/superpowers');
    });

    it('should return hasLegacyArtifacts: true when superpowers/AGENTS.md is found', async () => {
      await fs.writeFile(path.join(testDir, 'superpowers', 'AGENTS.md'), 'content');
    ...
    it('should detect project.md for migration hint (it is preserved, not deleted)', async () => {
      await fs.writeFile(path.join(testDir, 'superpowers', 'project.md'), 'content');
  ...
  describe('cleanupLegacyArtifacts', () => {
    it('should remove markers from config files that have only Superpowers content (never delete)', async () => {
      const claudePath = path.join(testDir, 'CLAUDE.md');
      await fs.writeFile(claudePath, `${SUPERPOWERS_MARKERS.start}\nContent\n${SUPERPOWERS_MARKERS.end}`);
      ...
      expect(content).not.toContain(SUPERPOWERS_MARKERS.start);
      expect(content).not.toContain(SUPERPOWERS_MARKERS.end);
    });
...
${SUPERPOWERS_MARKERS.end}`);
    ...
    it('should delete legacy slash command directories', async () => {
      ...
      expect(result.deletedDirs).toContain('.claude/commands/superpowers');
    ...
    it('should delete legacy slash command files', async () => {
      const dirPath = path.join(testDir, '.cursor', 'commands');
      await fs.mkdir(dirPath, { recursive: true });
      const filePath = path.join(dirPath, 'superpowers-proposal.md');
      await fs.writeFile(filePath, 'content');

      const detection = await detectLegacyArtifacts(testDir);
      const result = await cleanupLegacyArtifacts(testDir, detection);

      expect(result.deletedFiles).toContain('.cursor/commands/superpowers-proposal.md');
      await expect(fs.access(filePath)).rejects.toThrow();
    });

    it('should delete superpowers/AGENTS.md', async () => {
      const agentsPath = path.join(testDir, 'superpowers', 'AGENTS.md');
      ...
      expect(result.deletedFiles).toContain('superpowers/AGENTS.md');
      await expect(fs.access(agentsPath)).rejects.toThrow();
      // superpowers directory should still exist
      await expect(fs.access(path.join(testDir, 'superpowers'))).resolves.not.toThrow();
    });

    it('should NOT delete superpowers/project.md', async () => {
      const projectPath = path.join(testDir, 'superpowers', 'project.md');
      ...
      expect(result.deletedFiles).not.toContain('superpowers/project.md');
...
${SUPERPOWERS_MARKERS.end}`);
    ...
    it('should remove markers from root AGENTS.md even when only Superpowers content (never delete)', async () => {
      const agentsPath = path.join(testDir, 'AGENTS.md');
      await fs.writeFile(agentsPath, `${SUPERPOWERS_MARKERS.start}\nSuperpowers content\n${SUPERPOWERS_MARKERS.end}`);
  ...
  describe('formatCleanupSummary', () => {
    ...
    it('should format deleted directories', () => {
      const result = {
        deletedFiles: [],
        modifiedFiles: [],
        deletedDirs: ['.claude/commands/superpowers'],
        projectMdNeedsMigration: false,
        errors: [],
      };

      const summary = formatCleanupSummary(result);
      expect(summary).toContain('✓ Removed .claude/commands/superpowers/ (replaced by /sp:*)');
    });
    ...
    it('should format modified files', () => {
      ...
      const summary = formatCleanupSummary(result);
      expect(summary).toContain('✓ Removed Superpowers markers from AGENTS.md');
    });
    ...
    it('should include migration hint for project.md', () => {
      ...
      expect(summary).toContain('superpowers/project.md');
  ...
  describe('formatDetectionSummary', () => {
    it('should include welcoming upgrade header and explanation', () => {
      ...
      expect(summary).toContain('Upgrading to the new Superpowers');
    ...
    it('should format slash command directories', () => {
      const detection = {
        configFiles: [],
        configFilesToUpdate: [],
        slashCommandDirs: ['.claude/commands/superpowers'],
        slashCommandFiles: [],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
      ...
      expect(summary).toContain('• .claude/commands/superpowers/');
    });
    ...
    it('should format slash command files', () => {
      const detection = {
        ...
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: ['.cursor/commands/superpowers-proposal.md'],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
        hasLegacyArtifacts: true,
      ...
      const summary = formatDetectionSummary(detection);
      expect(summary).toContain('Files to remove');
      expect(summary).toContain('• .cursor/commands/superpowers-proposal.md');
    });

    it('should format superpowers/AGENTS.md', () => {
      const detection = {
        configFiles: [],
        configFilesToUpdate: [],
        slashCommandDirs: [],
      ...
      const summary = formatDetectionSummary(detection);
      expect(summary).toContain('Files to remove');
      expect(summary).toContain('• superpowers/AGENTS.md');
    });
    ...
    it('should group both removals and updates correctly', () => {
      ...
      expect(summary).toContain('Files to remove');
      expect(summary).toContain('Files to update');
      // Check removals (only slash commands and superpowers/AGENTS.md)
      expect(summary).toContain('• .claude/commands/superpowers/');
      expect(summary).toContain('• superpowers/AGENTS.md');
  ...
  describe('formatProjectMdMigrationHint', () => {
    it('should return migration hint message', () => {
      ...
      expect(hint).toContain('superpowers/project.md');
    ...
    it('should explain the new context section benefits', () => {
      const hint = formatProjectMdMigrationHint();
      expect(hint).toContain('included in every Superpowers request');
  ...
  describe('LEGACY_SLASH_COMMAND_PATHS', () => {
    it('should include expected tool patterns', () => {
      expect(LEGACY_SLASH_COMMAND_PATHS['claude']).toEqual({
        type: 'directory',
        path: '.claude/commands/superpowers',
      });

      expect(LEGACY_SLASH_COMMAND_PATHS['cursor']).toEqual({
        type: 'files',
        pattern: '.cursor/commands/superpowers-*.md',
      });

      expect(LEGACY_SLASH_COMMAND_PATHS['windsurf']).toEqual({
        type: 'files',
        pattern: '.windsurf/workflows/superpowers-*.md',
      });
    });
  ...
  describe('getToolsFromLegacyArtifacts', () => {
    ...
    it('should extract multiple tools from mixed legacy artifacts', () => {
      const detection = {
        configFiles: [],
        configFilesToUpdate: [],
        slashCommandDirs: ['.claude/commands/superpowers', '.qoder/commands/superpowers'],
        slashCommandFiles: ['.cursor/commands/superpowers-apply.md', '.windsurf/workflows/superpowers-archive.md'],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
        hasLegacyArtifacts: true,
    ...
    it('should deduplicate tools when multiple files match same tool', () => {
      const detection = {
        ...
        slashCommandDirs: [],
        slashCommandFiles: [
          '.cursor/commands/superpowers-proposal.md',
          '.cursor/commands/superpowers-apply.md',
          '.cursor/commands/superpowers-archive.md',
        ],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
    ...
    it('should handle qwen TOML-based legacy files', () => {
      const detection = {
        ...
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: ['.qwen/commands/superpowers-proposal.toml'],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
        hasLegacyArtifacts: true,
    ...
    it('should handle continue prompt files', () => {
      const detection = {
        ...
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: ['.continue/prompts/superpowers-apply.prompt'],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
        hasLegacyArtifacts: true,
    ...
    it('should handle github-copilot prompt files', () => {
      const detection = {
        ...
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: ['.github/prompts/superpowers-apply.prompt.md'],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
        hasLegacyArtifacts: true,
    ...
    it('should handle opencode sp-* legacy files', () => {
      const detection = {
        configFiles: [],
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: ['.opencode/command/sp-propose.md'],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
        hasLegacyArtifacts: true,
    ...
    it('should handle opencode superpowers-* legacy files', () => {
      const detection = {
        configFiles: [],
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: ['.opencode/command/superpowers-new.md'],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
        hasLegacyArtifacts: true,
    ...
    it('should deduplicate opencode when both sp-* and superpowers-* files exist', () => {
      const detection = {
        configFiles: [],
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: [
          '.opencode/command/sp-propose.md',
          '.opencode/command/superpowers-new.md',
        ],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
