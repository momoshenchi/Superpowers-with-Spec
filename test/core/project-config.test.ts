...
describe('project-config', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'superpowers-test-config-'));
...
`
      ...
      it('should handle completely invalid YAML gracefully', () => {
        ...
        expect(consoleWarnSpy).toHaveBeenCalledWith(
          expect.stringContaining('Failed to parse superpowers/config.yaml'),
          expect.anything()
        );
      });
...
`
    ...
    describe('.yml/.yaml precedence', () => {
      ...
      it('should return null when superpowers directory does not exist', () => {
        const config = readProjectConfig(tempDir);
  ...
  describe('suggestSchemas', () => {
    ...
    it('should include fix instruction', () => {
      ...
      expect(message).toContain(
        "Fix: Edit superpowers/config.yaml and change 'schema: wrong-schema' to a valid schema name"
      );
    });
