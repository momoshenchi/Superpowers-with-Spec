...
describe('ListCommand', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = path.join(os.tmpdir(), `superpowers-list-test-${Date.now()}`);
  ...
  describe('execute', () => {
    it('should handle missing superpowers/changes directory', async () => {
      const listCommand = new ListCommand();
      
      await expect(listCommand.execute(tempDir, 'changes')).rejects.toThrow(
        "No Superpowers changes directory found. Run 'superpowers init' first."
      );
    });

    it('should handle empty changes directory', async () => {
      const changesDir = path.join(tempDir, 'superpowers', 'changes');
