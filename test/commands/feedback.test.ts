...
describe('FeedbackCommand', () => {
  ...
  describe('gh CLI availability check', () => {
    it('should use which command on Unix/macOS platforms', async () => {
      ...
      mockExecFileSync.mockReturnValue('https://github.com/momoshenchi/Superpowers-with-Spec/issues/123\n');
    ...
    it('should handle missing gh CLI with fallback', async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining('https://github.com/momoshenchi/Superpowers-with-Spec/issues/new')
      );
    });
  ...
  describe('successful feedback submission', () => {
    it('should submit feedback via gh CLI when authenticated', async () => {
      const issueUrl = 'https://github.com/momoshenchi/Superpowers-with-Spec/issues/123';
      ...
      expect(mockExecFileSync).toHaveBeenCalledWith(
        ...
        [
          ...
          'create',
          '--repo',
          'momoshenchi/Superpowers-with-Spec',
          '--title',
          'Feedback: Great tool!',
          '--body',
          expect.stringContaining('Submitted via Superpowers CLI'),
          '--label',
          'feedback',
        ],
    ...
    it('should include --body flag when body is provided', async () => {
      const issueUrl = 'https://github.com/momoshenchi/Superpowers-with-Spec/issues/124';
    ...
    it('should format title with "Feedback:" prefix', async () => {
      ...
      mockExecFileSync.mockReturnValue('https://github.com/momoshenchi/Superpowers-with-Spec/issues/125\n');
    ...
    it('should include metadata in issue body', async () => {
      ...
      mockExecFileSync.mockReturnValue('https://github.com/momoshenchi/Superpowers-with-Spec/issues/126\n');
      ...
      expect(mockExecFileSync).toHaveBeenCalledWith(
        ...
        expect.arrayContaining([
          '--body',
          expect.stringMatching(/Submitted via Superpowers CLI[\s\S]*Version:[\s\S]*Platform:[\s\S]*Timestamp:/),
        ]),
        expect.any(Object)
      );
    });
    ...
    it('should add feedback label to the issue', async () => {
      ...
      mockExecFileSync.mockReturnValue('https://github.com/momoshenchi/Superpowers-with-Spec/issues/127\n');
  ...
  describe('error handling', () => {
    ...
    it('should handle quotes in title and body without escaping (no shell injection)', async () => {
      ...
      mockExecFileSync.mockReturnValue('https://github.com/momoshenchi/Superpowers-with-Spec/issues/128\n');
  ...
  describe('formatted feedback output', () => {
    ...
    it('should generate correct manual submission URL', async () => {
      ...
      const urlCall = consoleLogSpy.mock.calls.find((call: any[]) =>
        call[0]?.includes('https://github.com/momoshenchi/Superpowers-with-Spec/issues/new')
      );
