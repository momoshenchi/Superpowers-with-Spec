...
describe('FileSystemUtils.updateFileWithMarkers', () => {
  let testDir: string;
  const START_MARKER = '<!-- SUPERPOWERS:START -->';
  const END_MARKER = '<!-- SUPERPOWERS:END -->';

  beforeEach(async () => {
    testDir = path.join(os.tmpdir(), `superpowers-marker-test-${Date.now()}`);
  ...
  describe('new file creation', () => {
    it('should create new file with markers and content', async () => {
      ...
      const content = 'Superpowers content';
  ...
  describe('existing file without markers', () => {
    it('should prepend markers and content to existing file', async () => {
      ...
      const newContent = 'Superpowers content';
      await FileSystemUtils.updateFileWithMarkers(
        filePath,
        newContent,
        START_MARKER,
  ...
  describe('existing file with markers', () => {
    it('should replace content between markers', async () => {
      ...
      const oldManagedContent = 'Old Superpowers content';
      const afterContent = '# After\nSome content after';
      
      const existingFile = `${beforeContent}\n${START_MARKER}\n${oldManagedContent}\n${END_MARKER}\n${afterContent}`;
      await fs.writeFile(filePath, existingFile);
      
      const newContent = 'New Superpowers content';
      await FileSystemUtils.updateFileWithMarkers(
        filePath,
        newContent,
        START_MARKER,
...
After block content`;
  ...
  describe('shell markers', () => {
    const SHELL_START = '# SUPERPOWERS:START';
    const SHELL_END = '# SUPERPOWERS:END';
...
${SHELL_START}
# Superpowers managed
alias superpowers="npx superpowers"
${SHELL_END}
...
export EDITOR="vim"`;
      ...
      expect(result).not.toContain('alias superpowers');
