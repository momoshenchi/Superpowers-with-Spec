...
function main() {
  ...
  try {
    log(`Packing @momoshenchi/superpowers@${expected}...`);
    const filename = npmPack();
    tgzPath = path.resolve(filename);
    log(`Created: ${tgzPath}`);

    work = mkdtempSync(path.join(tmpdir(), 'superpowers-pack-check-'));
    log(`Temp dir: ${work}`);
    ...
    const binRel = path.join('node_modules', '@momoshenchi', 'superpowers', 'bin', 'superpowers.js');
