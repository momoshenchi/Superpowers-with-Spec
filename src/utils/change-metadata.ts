...
/**
 * Writes change metadata to .superpowers.yaml in the change directory.
 *
...
/**
 * Reads change metadata from .superpowers.yaml in the change directory.
 *
 * @param changeDir - The path to the change directory
 * @param projectRoot - Optional project root for project-local schema resolution
 ...
 * 2. Schema from .superpowers.yaml metadata (if exists)
 * 3. Schema from superpowers/config.yaml (if exists)
...
export function resolveSchemaForChange(
  ...
  // Derive project root from changeDir (changeDir is typically projectRoot/superpowers/changes/change-name)
