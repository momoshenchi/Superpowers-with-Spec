...
export class ShowCommand {
  ...
  private async showDirect(itemName: string, params: { typeOverride?: ItemType; options: { json?: boolean; [k: string]: any } }): Promise<void> {
    ...
    if (!params.typeOverride && isChange && isSpec) {
      ...
      console.error('Pass --type change|spec, or use: superpowers change show / superpowers spec show');
      process.exitCode = 1;
      return;
    }
  ...
  private printNonInteractiveHint(): void {
    ...
    console.error('  superpowers show <item>');
    console.error('  superpowers change show');
    console.error('  superpowers spec show');
