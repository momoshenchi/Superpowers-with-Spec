    ...
    .action(async (
      ...
      try {
          ...
          if (!options?.force) {
            if (options?.json) {
              console.log(JSON.stringify({
                ...
                suggestion: 'Use --force to overwrite or "superpowers schema fork" to copy',
              }, null, 2));
            } else {
              console.error(`Error: Schema '${name}' already exists at ${schemaDir}`);
              console.error('Use --force to overwrite or "superpowers schema fork" to copy');
            }
            process.exitCode = 1;
            return;
          }
        ...
        if (options?.json) {
          ...
          console.log(`  3. Use with: superpowers new --schema ${name}`);
        }
      } catch (error) {
