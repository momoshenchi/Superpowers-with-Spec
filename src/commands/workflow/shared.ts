...
/**
 * Returns the list of available change directory names under superpowers/changes/.
...
export async function getAvailableChanges(projectRoot: string): Promise<string[]> {
  const changesPath = path.join(projectRoot, 'superpowers', 'changes');
  try {
...
export async function validateChangeExists(
  ...
  if (!exists) {
    ...
    if (available.length === 0) {
      throw new Error(
        `Change '${changeName}' not found. No changes exist. Create one with: superpowers new change <name>`
      );
    }
    throw new Error(
