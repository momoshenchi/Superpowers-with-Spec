...
const SPECS_DIR = 'superpowers/specs';
...
function filterSpec(spec: Spec, options: ShowOptions): Spec {
  ...
  const metadata = spec.metadata ?? { version: '1.0.0', format: 'superpowers' as const };
...
export class SpecCommand {
  private SPECS_DIR = 'superpowers/specs';
  ...
  async show(specId?: string, options: ShowOptions = {}): Promise<void> {
    ...
    if (!existsSync(specPath)) {
      throw new Error(`Spec '${specId}' not found at superpowers/specs/${specId}/spec.md`);
    }
    ...
    if (options.json) {
      ...
      const output = {
        ...
        metadata: parsed.metadata ?? { version: '1.0.0', format: 'superpowers' as const },
      };
...
export function registerSpecCommand(rootProgram: typeof program) {
  const specCommand = rootProgram
    .command('spec')
    .description('Manage and view Superpowers specifications');

  // Deprecation notice for noun-based commands
  specCommand.hook('preAction', () => {
    console.error('Warning: The "superpowers spec ..." commands are deprecated. Prefer verb-first commands (e.g., "superpowers show", "superpowers validate --specs").');
  });
