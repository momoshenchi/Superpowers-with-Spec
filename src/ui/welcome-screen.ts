...
function getWelcomeText(): string[] {
  return [
    chalk.white.bold('Welcome to Superpowers'),
    chalk.dim('A lightweight spec-driven framework'),
    '',
    chalk.white('This setup will configure:'),
    chalk.dim('  • Agent Skills for AI tools'),
    chalk.dim('  • /sp:* slash commands'),
    '',
    chalk.white('Quick start after setup:'),
    `  ${chalk.yellow('/sp:new')}      ${chalk.dim('Create a change')}`,
    `  ${chalk.yellow('/sp:continue')} ${chalk.dim('Next artifact')}`,
    `  ${chalk.yellow('/sp:apply')}    ${chalk.dim('Implement tasks')}`,
    '',
