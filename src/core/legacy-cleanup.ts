/**
 * Legacy cleanup module for detecting and removing Superpowers artifacts
...
import { SUPERPOWERS_MARKERS } from './config.js';

/**
 * Legacy config file names from the old ToolRegistry.
 * These were config files created at project root with Superpowers markers.
 */
export const LEGACY_CONFIG_FILES = [
  ...
  'QODER.md',
  'IFLOW.md',
  'AGENTS.md', // root AGENTS.md (not superpowers/AGENTS.md)
  'QWEN.md',
] as const;
...
export const LEGACY_SLASH_COMMAND_PATHS: Record<string, LegacySlashCommandPattern> = {
  // Directory-based: .tooldir/commands/superpowers/ or .tooldir/commands/superpowers/*.md
  'claude': { type: 'directory', path: '.claude/commands/superpowers' },
  'codebuddy': { type: 'directory', path: '.codebuddy/commands/superpowers' },
  'qoder': { type: 'directory', path: '.qoder/commands/superpowers' },
  'crush': { type: 'directory', path: '.crush/commands/superpowers' },
  'gemini': { type: 'directory', path: '.gemini/commands/superpowers' },
  'costrict': { type: 'directory', path: '.cospec/superpowers/commands' },

  // File-based: individual superpowers-*.md files in a commands/workflows/prompts folder
  'cursor': { type: 'files', pattern: '.cursor/commands/superpowers-*.md' },
  'windsurf': { type: 'files', pattern: '.windsurf/workflows/superpowers-*.md' },
  'kilocode': { type: 'files', pattern: '.kilocode/workflows/superpowers-*.md' },
  'kiro': { type: 'files', pattern: '.kiro/prompts/superpowers-*.prompt.md' },
  'github-copilot': { type: 'files', pattern: '.github/prompts/superpowers-*.prompt.md' },
  'amazon-q': { type: 'files', pattern: '.amazonq/prompts/superpowers-*.md' },
  'cline': { type: 'files', pattern: '.clinerules/workflows/superpowers-*.md' },
  'roocode': { type: 'files', pattern: '.roo/commands/superpowers-*.md' },
  'auggie': { type: 'files', pattern: '.augment/commands/superpowers-*.md' },
  'factory': { type: 'files', pattern: '.factory/commands/superpowers-*.md' },
  'opencode': { type: 'files', pattern: ['.opencode/command/sp-*.md', '.opencode/command/superpowers-*.md'] },
  'continue': { type: 'files', pattern: '.continue/prompts/superpowers-*.prompt' },
  'antigravity': { type: 'files', pattern: '.agent/workflows/superpowers-*.md' },
  'iflow': { type: 'files', pattern: '.iflow/commands/superpowers-*.md' },
  'qwen': { type: 'files', pattern: '.qwen/commands/superpowers-*.toml' },
  'codex': { type: 'files', pattern: '.codex/prompts/superpowers-*.md' },
};
...
export interface LegacyDetectionResult {
  /** Config files with Superpowers markers detected */
  configFiles: string[];
  ...
  /** Whether superpowers/AGENTS.md exists */
  hasOpenspecAgents: boolean;
  /** Whether superpowers/project.md exists (preserved, migration hint only) */
  hasProjectMd: boolean;
  /** Whether root AGENTS.md has Superpowers markers */
...
/**
 * Detects all legacy Superpowers artifacts in a project.
 *
...
/**
 * Detects legacy config files with Superpowers markers.
...
export async function detectLegacyConfigFiles(
  ...
  for (const fileName of LEGACY_CONFIG_FILES) {
    ...
    if (await FileSystemUtils.fileExists(filePath)) {
      ...
      if (hasSuperpowersMarkers(content)) {
 ...
 *
 * @param projectPath - The root path of the project
 * @param pattern - Glob pattern like '.cursor/commands/superpowers-*.md'
...
async function findLegacySlashCommandFiles(
  ...
  try {
    ...
    // superpowers-*.prompt.md -> /^superpowers-.*\.prompt\.md$/
    // superpowers-*.toml -> /^superpowers-.*\.toml$/
...
/**
 * Detects legacy Superpowers structure files (AGENTS.md and project.md).
 *
 * @param projectPath - The root path of the project
...
export async function detectLegacyStructureFiles(
  ...
  // Check for superpowers/AGENTS.md
  const superpowersAgentsPath = FileSystemUtils.joinPath(projectPath, 'superpowers', 'AGENTS.md');
  hasOpenspecAgents = await FileSystemUtils.fileExists(superpowersAgentsPath);

  // Check for superpowers/project.md (for migration messaging, not deleted)
  const projectMdPath = FileSystemUtils.joinPath(projectPath, 'superpowers', 'project.md');
  hasProjectMd = await FileSystemUtils.fileExists(projectMdPath);

  // Check for root AGENTS.md with Superpowers markers
  ...
  if (await FileSystemUtils.fileExists(rootAgentsPath)) {
    ...
    hasRootAgentsWithMarkers = hasSuperpowersMarkers(content);
  }
...
/**
 * Checks if content contains Superpowers markers.
 *
 * @param content - File content to check
 * @returns True if both start and end markers are present
 */
export function hasSuperpowersMarkers(content: string): boolean {
  return (
    content.includes(SUPERPOWERS_MARKERS.start) && content.includes(SUPERPOWERS_MARKERS.end)
  );
}

/**
 * Checks if file content is 100% Superpowers content (only markers and whitespace outside).
 *
 * @param content - File content to check
 * @returns True if content outside markers is only whitespace
 */
export function isOnlySuperpowersContent(content: string): boolean {
  const startIndex = content.indexOf(SUPERPOWERS_MARKERS.start);
  const endIndex = content.indexOf(SUPERPOWERS_MARKERS.end);
  ...
  const after = content.substring(endIndex + SUPERPOWERS_MARKERS.end.length);

  return before.trim() === '' && after.trim() === '';
}

/**
 * Removes the Superpowers marker block from file content.
 ...
 *
 * @param content - File content with Superpowers markers
 * @returns Content with marker block removed
 */
export function removeMarkerBlock(content: string): string {
  return removeMarkerBlockUtil(content, SUPERPOWERS_MARKERS.start, SUPERPOWERS_MARKERS.end);
}
...
/**
 * Cleans up legacy Superpowers artifacts from a project.
 * Preserves superpowers/project.md (shows migration hint instead of deleting).
 *
 * @param projectPath - The root path of the project
...
export async function cleanupLegacyArtifacts(
  ...
  // Delete legacy slash command directories (these are 100% Superpowers-managed)
  ...
  // Delete legacy slash command files (these are 100% Superpowers-managed)
  ...
  // Delete superpowers/AGENTS.md (this is inside superpowers/, it's Superpowers-managed)
  if (detection.hasOpenspecAgents) {
    const agentsPath = FileSystemUtils.joinPath(projectPath, 'superpowers', 'AGENTS.md');
    if (await FileSystemUtils.fileExists(agentsPath)) {
      try {
        await fs.unlink(agentsPath);
        result.deletedFiles.push('superpowers/AGENTS.md');
      } catch (error: any) {
        result.errors.push(`Failed to delete superpowers/AGENTS.md: ${error.message}`);
      }
    }
  }

  // Handle root AGENTS.md with Superpowers markers - remove markers only, NEVER delete
...
export function formatCleanupSummary(result: CleanupResult): string {
  ...
  if (result.deletedFiles.length > 0 || result.deletedDirs.length > 0 || result.modifiedFiles.length > 0) {
    ...
    for (const dir of result.deletedDirs) {
      lines.push(`  ✓ Removed ${dir}/ (replaced by /sp:*)`);
    }

    for (const file of result.modifiedFiles) {
      lines.push(`  ✓ Removed Superpowers markers from ${file}`);
    }
  }
 ...
 * Only includes Superpowers-managed files (slash commands, superpowers/AGENTS.md).
...
function buildRemovalsList(detection: LegacyDetectionResult): Array<{ path: string; explanation: string }> {
  ...
  // Slash command directories (these are 100% Superpowers-managed)
  ...
  // Slash command files (these are 100% Superpowers-managed)
  for (const file of detection.slashCommandFiles) {
    removals.push({ path: file, explanation: 'replaced by skills/' });
  }

  // superpowers/AGENTS.md (inside superpowers/, it's Superpowers-managed)
  if (detection.hasOpenspecAgents) {
    removals.push({ path: 'superpowers/AGENTS.md', explanation: 'obsolete workflow file' });
  }
...
function buildUpdatesList(detection: LegacyDetectionResult): Array<{ path: string; explanation: string }> {
  ...
  for (const file of detection.configFilesToUpdate) {
    updates.push({ path: file, explanation: 'removing Superpowers markers' });
  }
...
export function formatDetectionSummary(detection: LegacyDetectionResult): string {
  ...
  lines.push(chalk.bold('Upgrading to the new Superpowers'));
  lines.push('');
  lines.push('Superpowers now uses agent skills, the emerging standard across coding');
  ...
  if (updates.length > 0) {
    ...
    lines.push(chalk.dim('Superpowers markers will be removed, your content preserved:'));
  ...
  for (const file of detection.slashCommandFiles) {
    ...
    for (const [toolId, pattern] of Object.entries(LEGACY_SLASH_COMMAND_PATHS)) {
      if (pattern.type === 'files' && pattern.pattern) {
        ...
        // e.g., '.cursor/commands/superpowers-*.md' -> /^\.cursor\/commands\/superpowers-.*\.md$/
        const patterns = Array.isArray(pattern.pattern) ? pattern.pattern : [pattern.pattern];
        let matched = false;
        for (const p of patterns) {
...
export function formatProjectMdMigrationHint(): string {
  ...
  lines.push('  • superpowers/project.md');
  lines.push(chalk.dim('    We won\'t delete this file. It may contain useful project context.'));
  lines.push('');
  lines.push(chalk.dim('    The new superpowers/config.yaml has a "context:" section for planning'));
  lines.push(chalk.dim('    context. This is included in every Superpowers request and works more'));
