...
# Use superpowers __complete to get available changes
_superpowers_complete_changes() {
  local -a changes
  while IFS=$'\\t' read -r id desc; do
    changes+=("$id:$desc")
  done < <(superpowers __complete changes 2>/dev/null)
  _describe "change" changes
}

# Use superpowers __complete to get available specs
_superpowers_complete_specs() {
  local -a specs
  while IFS=$'\\t' read -r id desc; do
    specs+=("$id:$desc")
  done < <(superpowers __complete specs 2>/dev/null)
