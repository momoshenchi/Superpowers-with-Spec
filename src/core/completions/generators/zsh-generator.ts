...
/**
 * Generates Zsh completion scripts for the Superpowers CLI.
 * Follows Zsh completion system conventions using the _superpowers function.
 */
export class ZshGenerator implements CompletionGenerator {
  ...
  generate(commands: CommandDefinition[]): string {
    ...
    for (const cmd of commands) {
      ...
      commandCaseLines.push(`          _superpowers_${this.sanitizeFunctionName(cmd.name)}`);
    ...
    return `#compdef superpowers

# Zsh completion script for Superpowers CLI
...
${commandList}
  ...
  case $state in
    command)
      _describe "superpowers command" commands
      ;;
    args)
      case $words[1] in
${commandCases}
...
${commandFunctions}
${helpers}
compdef _superpowers superpowers
`;
  }
  ...
  private generateCommandFunction(cmd: CommandDefinition): string[] {
    const funcName = `_superpowers_${this.sanitizeFunctionName(cmd.name)}`;
    const lines: string[] = [];
    ...
    if (cmd.subcommands && cmd.subcommands.length > 0) {
      ...
      for (const subcmd of cmd.subcommands) {
        ...
        lines.push(`          _superpowers_${this.sanitizeFunctionName(cmd.name)}_${this.sanitizeFunctionName(subcmd.name)}`);
        lines.push('          ;;');
      }
  ...
  private generateSubcommandFunction(parentName: string, subcmd: CommandDefinition): string[] {
    const funcName = `_superpowers_${this.sanitizeFunctionName(parentName)}_${this.sanitizeFunctionName(subcmd.name)}`;
    const lines: string[] = [];
  ...
  private generatePositionalSpec(positionalType?: string): string {
    switch (positionalType) {
      case 'change-id':
        return "'*: :_superpowers_complete_changes'";
      case 'spec-id':
        return "'*: :_superpowers_complete_specs'";
      case 'change-or-spec-id':
        return "'*: :_superpowers_complete_items'";
      case 'path':
        return "'*:path:_files'";
      case 'shell':
