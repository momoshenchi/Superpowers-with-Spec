...
const REVIEWER_PROCEDURE = `1. Run \`superpowers status --change "<name>" --json\`. Read \`schemaName\`, \`applyRequires\`, and \`artifacts[]\` (\`id\`, \`outputPath\`, \`status\`). Cross-check \`.superpowers.yaml\` \`schema:\` when helpful. Do not assume every schema has delta specs.
...
Do not reuse a review worker across rounds. Prefer a fresh reviewer. Do not perform inspection and dimensional assessment in the coordinator context when the host can launch subagents.
If the host cannot launch a subagent, the coordinator runs the four dimensions in the current context and labels the review mode \`same-context fallback\`. Missing spawn capability is not by itself \`blocked\`. \`P0\` findings still withhold readiness.
...
| \`execution-plan.md\` | \`spec-driven\` | File Structure; stepwise Task Plan (red test → implement → verify); every Step 1 filling spec bound, test location, paste-ready test skeleton; every Step 3 filling change anchor, design carried, implementation approach, edges and failures, out of scope; optional non-normative Implementation Notes | drift from \`tasks.md\`; missing commands and expected signals; Step 1 that only says "cover the behavior" or has no skeleton; Step 3 that only restates the task title or leaves a slot empty |
| \`test-plan.md\` | all | Testing Gap Analysis; Test Scope Register taken from each \`### Requirement:\` (R1…Rn) with Entry Point as the system under test and existing \`#### Scenario:\` titles as the import list; Design Contract And Invariant Coverage; Requirement/Scenario coverage matrix that maps each imported Scenario to a D1 Case ID (Form, Status, Steps, and Expected live on that D1 row, not on the index); Six-Dimension Case Matrix split into one table per \`full-qa-test\` D1–D6 dimension with \`TC-R<n>-D<m>-<seq>\` IDs, Object, Source on D1 (\`imported:\` / \`gap:\`), and Form; Dimension Coverage Summary; post-implementation Test Hardening notes when applicable | happy path only; invents a parallel feature list instead of using spec Requirements; treats a thin spec as the coverage ceiling; treats a file or Scenario as the object; copies RTM rows into D1 with the same Form/Status; imports Scenarios and stops; shares one batch of ten across several Requirements; cases that never name which Requirement they exercise or which form they use; E2E used where a unit or integration test can observe the behavior; vague expectations such as "invalid input is rejected"; a registered Requirement absent from an applicable dimension without a stated reason |

Review MAY warn on missing risk dimensions. An unfinished 10-per-dimension batch is not a completeness BLOCKER.
