...
- **Invariants**: expect exact \`## Invariants\`. Accept \`N/A — no cross-path invariants\` when none apply. Missing \`## Invariants\` heading → **BLOCKER**. Empty body with neither rows nor N/A → **BLOCKER**. Non-N/A rows lacking stable ID, falsifiable statement, how-to-falsify, or owner test/check → WARNING. Auth/money/data-integrity changes with only N/A → WARNING (recommend ≥1 invariant).
...
- **User-real choices**: \`**User selection:**\` tables are allowed only for choices the user actually chose. An A/B/C table labeled as user Choice when the user did not choose → WARNING (**misattributed user Choice**). Agent-owned A/B/C is allowed; a one-line or ritual Choice without why winners/losers → WARNING (**shallow rationale**). Agent-owned A/B/C with **strict, detailed analysis** → **not a finding**. Missing three-option tables → **not a finding**.
- **Implementable detail**: a behavioral change whose design is **principle-only** (no target flow, mapping rules, or **worked example**) → WARNING; escalate to BLOCKER when that would block implementation. Extra subsections under existing headings are welcome. Missing invented top-level headings such as Target flow → **not a finding**.
- **Closed implication scan**: for a behavioral change, expect a derived rule or short N/A for each of: Actor, permission, and ownership; Empty, deny, error, and fail-closed behavior; Lifecycle: create, update, cancel, retry, and idempotency; Compatibility and migration; Data shape and contracts; Important product-direction forks implied by the confirmed goal. Coverage belongs in existing \`## Decisions\`, \`## Contracts\`, and \`## Invariants\`. A displayed Derived implications heading is not required → **not a finding**. A local helper MAY cover the scan with one short N/A. Missing dimension (neither rule nor N/A) → **WARNING**. never BLOCKER solely for a derived-implication gap. A user-visible derived rule without a delta-spec trace → **WARNING**. Derived items labeled as a user Choice remain **misattributed user Choice**.
