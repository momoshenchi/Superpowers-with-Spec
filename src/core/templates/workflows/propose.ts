...
const PROPOSE_ARTIFACT_LOOP = `# Artifact generation loop
...
const PROPOSE_ARTIFACT_LOOP_POINTER = `4. **Create artifacts in sequence until apply-ready** — after authorized create, read \`reference/artifact-loop.md\`. If that file is missing, report the stage \`blocked\` with the missing path; do not skip. Do not inline the per-artifact instruction loop here.
`;

export function getProposeReferences(): SkillReferenceFile[] {
  return [{ relativePath: 'reference/artifact-loop.md', content: PROPOSE_ARTIFACT_LOOP }];
}
...
`,
    ...
    references: getProposeReferences(),
  };
}
