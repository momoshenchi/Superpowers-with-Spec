...
## Purpose
Provide shell completion scripts for the Superpowers CLI, enabling tab-completion for commands, flags, and dynamic values (change IDs, spec IDs) across multiple shells. Supports Zsh, Bash, Fish, and PowerShell.
## Requirements
...
- **WHEN** user executes `superpowers completion --help`
...
- **WHEN** user executes `superpowers completion generate zsh`
...
- **WHEN** user executes `superpowers completion generate bash`
...
- **AND** support dynamic completion for change and spec IDs via `superpowers __complete`

#### Scenario: Generating Fish completion

- **WHEN** user executes `superpowers completion generate fish`
- **THEN** output a complete Fish completion script to stdout
- **AND** use `complete -c superpowers` with conditions
...
- **WHEN** user executes `superpowers completion generate powershell`
- **THEN** output a complete PowerShell completion script to stdout
- **AND** use `Register-ArgumentCompleter -CommandName superpowers`
...
- **THEN** discover active changes from `superpowers/changes/` directory
- **AND** exclude archived changes in `superpowers/changes/archive/`
- **AND** return change IDs as completion suggestions
- **AND** only provide suggestions when inside an Superpowers-enabled project

#### Scenario: Completing spec IDs

- **WHEN** completing arguments for commands that accept spec names (show, validate)
- **THEN** discover specs from `superpowers/specs/` directory
...
- **WHEN** user requests completions outside an Superpowers project
...
- **WHEN** user executes `superpowers completion install zsh`
...
- **AND** write completion script to `~/.oh-my-zsh/custom/completions/_superpowers`
...
- **WHEN** user executes `superpowers completion install zsh` and Oh My Zsh is not detected
- **THEN** create completions directory at `~/.zsh/completions/` if it doesn't exist
- **AND** write completion script to `~/.zsh/completions/_superpowers`
...
- **WHEN** user executes `superpowers completion install bash`
- **THEN** detect if bash-completion is installed by checking for `/usr/share/bash-completion` or `/etc/bash_completion.d`
- **AND** if bash-completion is available, write to `/etc/bash_completion.d/superpowers` (with sudo) or `~/.local/share/bash-completion/completions/superpowers`
- **AND** if bash-completion is not available, write to `~/.bash_completion.d/superpowers` and source it from `~/.bashrc`
...
- **WHEN** user executes `superpowers completion install fish`
- **THEN** create Fish completions directory at `~/.config/fish/completions/` if it doesn't exist
- **AND** write completion script to `~/.config/fish/completions/superpowers.fish`
...
- **WHEN** user executes `superpowers completion install powershell`
...
- **WHEN** user executes `superpowers completion install` without specifying a shell
...
- **WHEN** user executes `superpowers completion uninstall zsh`
...
- **AND** if user confirms, remove `~/.oh-my-zsh/custom/completions/_superpowers` if Oh My Zsh is detected
- **AND** remove `~/.zsh/completions/_superpowers` if standard Zsh setup is detected
- **AND** remove fpath modifications from `~/.zshrc` using marker-based removal
- **AND** display success message

#### Scenario: Uninstalling Bash completion

- **WHEN** user executes `superpowers completion uninstall bash`
...
- **WHEN** user executes `superpowers completion uninstall fish`
- **THEN** prompt for confirmation (unless `--yes` flag provided)
- **AND** if user confirms, remove `~/.config/fish/completions/superpowers.fish`
- **AND** display success message (no config file modification needed)

#### Scenario: Uninstalling PowerShell completion

- **WHEN** user executes `superpowers completion uninstall powershell`
- **THEN** prompt for confirmation (unless `--yes` flag provided)
...
- **WHEN** user executes `superpowers completion uninstall` without specifying a shell
...
- **AND** implement methods:
  ...
  - `isSuperpowersProject(): boolean` - Checks if current directory is Superpowers-enabled
...
- **WHEN** `superpowers completion install` cannot detect current shell
- **THEN** display error: "Could not auto-detect shell. Please specify shell explicitly."
- **AND** display usage hint: "Usage: superpowers completion <operation> [shell]"
- **AND** exit with code 1
...
- **AND** allow redirection to files: `superpowers completion generate zsh > /path/to/_superpowers`
