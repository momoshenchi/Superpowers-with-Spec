# Superpowers Conventions Specification

## Purpose

Superpowers conventions SHALL define how system capabilities are documented, how changes are proposed and tracked, and how specifications evolve over time. This meta-specification serves as the source of truth for Superpowers's own conventions.
## Requirements
### Requirement: Structured conventions for specs and changes

Superpowers conventions SHALL mandate a structured spec format with clear requirement and scenario sections so tooling can parse consistently.

#### Scenario: Following the structured spec format

- **WHEN** writing or updating Superpowers specifications
...
Superpowers specifications SHALL capture verifiable behavior contracts and avoid internal implementation detail.
...
Superpowers conventions SHALL keep specs lightweight by default and scale rigor only when risk or coordination complexity demands it.
...
An Superpowers project SHALL maintain a consistent directory structure for specifications and changes.

#### Scenario: Initializing project structure
- **WHEN** an Superpowers project is initialized
...
Superpowers CLI design SHALL use verbs as top-level commands with nouns provided as arguments or flags for scoping.

#### Scenario: Verb-first command discovery
- **WHEN** a user runs a command like `superpowers list`
...
- **WHEN** users run noun-prefixed commands such as `superpowers spec ...` or `superpowers change ...`
...
- **THEN** `superpowers show` and `superpowers validate` SHALL accept `--type spec|change`
