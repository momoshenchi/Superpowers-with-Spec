...
## Purpose
Define `superpowers validate` behavior for validating changes and specs with actionable remediation guidance and structured output.
...
- **THEN** show error "No deltas found" with guidance:
  ...
  - Remind authors that files must live under `superpowers/changes/{id}/specs/<capability>/spec.md`
  - Include an explicit note: "Spec delta files cannot start with titles before the operation headers"
  - Suggest running `superpowers change show {id} --json --deltas-only` for debugging
...
- **THEN** include expected header names and a minimal skeleton:
  ...
  - Mention the quick-reference section in `superpowers/AGENTS.md` as the authoritative template
...
- **THEN** emit an error explaining that `### Requirement:` lines must be followed by narrative text before any `#### Scenario:` headers
  ...
  - Reference the pre-validation checklist in `superpowers/AGENTS.md`
...
- Source file path (`superpowers/changes/{id}/proposal.md`, `.../specs/{cap}/spec.md`)
...
- **THEN** print "Next steps" with 2-3 targeted bullets and suggest `superpowers change show <id> --json --deltas-only`
...
- **WHEN** executing `superpowers validate` without arguments
...
- **WHEN** executing `superpowers validate <item-name>`
...
- **WHEN** executing `superpowers validate --all`
- **THEN** validate all changes in superpowers/changes/ (excluding archive)
- **AND** validate all specs in superpowers/specs/
...
- **THEN** include all change proposals under `superpowers/changes/`
- **AND** exclude the `superpowers/changes/archive/` directory

- **WHEN** validating with `--specs`
- **THEN** include all specs that have a `spec.md` under `superpowers/specs/<id>/spec.md`

#### Scenario: Validate all changes

- **WHEN** executing `superpowers validate --changes`
- **THEN** validate all changes in superpowers/changes/ (excluding archive)
...
- **WHEN** executing `superpowers validate --specs`
- **THEN** validate all specs in superpowers/specs/
...
- **WHEN** executing `superpowers validate --all --strict`
...
- **WHEN** executing `superpowers validate --all --json`
...
- **WHEN** executing `superpowers validate --all --json` (or `--changes` / `--specs`)
...
- **AND** suggest passing `--type change` or `--type spec`, or using `superpowers change validate` / `superpowers spec validate`
...
- **WHEN** executing `superpowers validate --type change <item>`
- **THEN** treat `<item>` as a change ID and validate it (skipping auto-detection)

- **WHEN** executing `superpowers validate --type spec <item>`
...
- **WHEN** `superpowers validate` is executed with `--no-interactive` or with environment `OPEN_SPEC_INTERACTIVE=0`
...
- **WHEN** running `superpowers validate <change-id>`
