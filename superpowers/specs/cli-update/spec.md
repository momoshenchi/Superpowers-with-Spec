...
As a developer using Superpowers, I want to update the Superpowers instructions in my project when new versions are released, so that I can benefit from improvements to AI agent instructions.
## Requirements
...
- **AND** if a root-level stub (`AGENTS.md`/`CLAUDE.md`) exists, refresh it so it points to `@/superpowers/AGENTS.md`

### Requirement: Prerequisites

The command SHALL require an existing Superpowers structure before allowing updates.

#### Scenario: Checking prerequisites

- **GIVEN** the command requires an existing `superpowers` directory (created by `superpowers init`)
- **WHEN** the `superpowers` directory does not exist
- **THEN** display error: "No Superpowers directory found. Run 'superpowers init' first."
...
- **AND** if a root-level stub exists, update the managed block content so it keeps directing teammates to `@/superpowers/AGENTS.md`

### Requirement: Tool-Agnostic Updates
The update command SHALL refresh Superpowers-managed files in a predictable manner while respecting each team's chosen tooling.
...
- **AND** update only the Superpowers-managed sections inside existing AI tool files, leaving user-authored content untouched
...
- **THEN** replace `superpowers/AGENTS.md` with the latest template
- **AND** if a root-level stub exists, refresh it so it still directs contributors to `@/superpowers/AGENTS.md`
...
- **WHEN** `.agent/workflows/` contains `superpowers-proposal.md`, `superpowers-apply.md`, and `superpowers-archive.md`
- **THEN** refresh the Superpowers-managed portion of each file so the workflow copy matches other tools while preserving the existing single-field `description` frontmatter
...
- **WHEN** `.codebuddy/commands/superpowers/` contains `proposal.md`, `apply.md`, and `archive.md`
...
- **AND** preserve any user customizations outside the Superpowers managed markers

#### Scenario: Updating slash commands for Cline
- **WHEN** `.clinerules/workflows/` contains `superpowers-proposal.md`, `superpowers-apply.md`, and `superpowers-archive.md`
- **THEN** refresh each file using shared templates
- **AND** include Cline-specific Markdown heading frontmatter
- **AND** ensure templates include instructions for the relevant workflow stage

#### Scenario: Updating slash commands for Continue
- **WHEN** `.continue/prompts/` contains `superpowers-proposal.prompt`, `superpowers-apply.prompt`, and `superpowers-archive.prompt`
- **THEN** refresh each file using shared templates
- **AND** ensure templates include instructions for the relevant workflow stage

#### Scenario: Updating slash commands for Crush
- **WHEN** `.crush/commands/` contains `superpowers/proposal.md`, `superpowers/apply.md`, and `superpowers/archive.md`
- **THEN** refresh each file using shared templates
...
- **WHEN** `.gemini/commands/superpowers/` contains `proposal.toml`, `apply.toml`, and `archive.toml`
- **THEN** refresh the body of each file using the shared proposal/apply/archive templates
- **AND** replace only the content between `<!-- SUPERPOWERS:START -->` and `<!-- SUPERPOWERS:END -->` markers inside the `prompt = """` block so the TOML framing (`description`, `prompt`) stays intact
...
- **WHEN** `.iflow/commands/` contains `superpowers-proposal.md`, `superpowers-apply.md`, and `superpowers-archive.md`
- **THEN** refresh each file using shared templates
...
- **WHEN** a user invokes `/superpowers:archive <change-id>` with a change ID
- **THEN** the template SHALL instruct the AI to validate the provided change ID against `superpowers list`
...
- **WHEN** a user invokes `/superpowers:archive` without providing a change ID
- **THEN** the template SHALL instruct the AI to identify the change ID from context or by running `superpowers list`
...
- **AND** the default directory name `superpowers` SHALL be used

## Success Criteria

Users SHALL be able to:
- Update Superpowers instructions with a single command
