...
## Purpose
Define how `superpowers/config.yaml` is discovered, parsed, validated, and exposed to callers with safe fallbacks.

## Requirements
### Requirement: Load project config from superpowers/config.yaml

The system SHALL read and parse the project configuration file located at `superpowers/config.yaml` relative to the project root.

#### Scenario: Valid config file exists
- **WHEN** `superpowers/config.yaml` exists with valid YAML content
- **THEN** system parses the file and returns a ProjectConfig object

#### Scenario: Config file does not exist
- **WHEN** `superpowers/config.yaml` does not exist
- **THEN** system returns null without error

#### Scenario: Config file has invalid YAML syntax
- **WHEN** `superpowers/config.yaml` contains malformed YAML
- **THEN** system logs a warning message and returns null

#### Scenario: Config file has valid YAML but invalid schema
- **WHEN** `superpowers/config.yaml` contains valid YAML that fails Zod schema validation
...
- **WHEN** `superpowers/config.yml` exists and `superpowers/config.yaml` does not exist
- **THEN** system reads from `superpowers/config.yml`

#### Scenario: Both .yaml and .yml exist
- **WHEN** both `superpowers/config.yaml` and `superpowers/config.yml` exist
- **THEN** system prefers `superpowers/config.yaml`
...
- **WHEN** config file has syntax errors and user runs `superpowers new change`
