...
## Purpose
Define `superpowers schema validate` behavior for validating schema syntax, structure, templates, and dependency graphs.

## Requirements
### Requirement: Schema validate checks schema structure
The CLI SHALL provide an `superpowers schema validate [name]` command that validates schema configuration and reports errors.

#### Scenario: Validate specific schema
- **WHEN** user runs `superpowers schema validate my-workflow`
...
- **WHEN** user runs `superpowers schema validate` without a name
- **THEN** system validates all schemas in `superpowers/schemas/`
- **AND** displays results for each schema
- **AND** exits with non-zero code if any schema is invalid

#### Scenario: Schema not found
- **WHEN** user runs `superpowers schema validate nonexistent`
...
- **WHEN** user runs `superpowers schema validate my-workflow` and `schema.yaml` has syntax errors
...
- **WHEN** user runs `superpowers schema validate my-workflow --json` and schema is valid
- **THEN** system outputs JSON with `valid: true`, `name`, and `path` fields

#### Scenario: JSON output for invalid schema
- **WHEN** user runs `superpowers schema validate my-workflow --json` and schema has errors
- **THEN** system outputs JSON with `valid: false` and `issues` array
- **AND** each issue includes `level`, `path`, and `message` fields
- **AND** format matches existing `superpowers validate` output structure
...
- **WHEN** user runs `superpowers schema validate my-workflow --verbose`
