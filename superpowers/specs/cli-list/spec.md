...
The `superpowers list` command SHALL provide developers with a quick overview of all active changes in the project, showing their names and task completion status.
## Requirements
### Requirement: Command Execution
The command SHALL scan and analyze either active changes or specs based on the selected mode.

#### Scenario: Scanning for changes (default)
- **WHEN** `superpowers list` is executed without flags
- **THEN** scan the `superpowers/changes/` directory for change directories
- **AND** exclude the `archive/` subdirectory from results
- **AND** parse each change's `tasks.md` file to count task completion

#### Scenario: Scanning for specs
- **WHEN** `superpowers list --specs` is executed
- **THEN** scan the `superpowers/specs/` directory for capabilities
...
- **WHEN** `superpowers/changes/` directory doesn't exist
- **THEN** display error: "No Superpowers changes directory found. Run 'superpowers init' first."
...
This command provides that visibility with minimal effort, following Superpowers's philosophy of simplicity and clarity.