...
- **WHEN** user runs `superpowers schema init my-workflow`
- **THEN** system creates directory `superpowers/schemas/my-workflow/`
...
- **WHEN** user runs `superpowers schema init "My Workflow"` (contains space)
...
- **WHEN** user runs `superpowers schema init existing-schema` and `superpowers/schemas/existing-schema/` already exists
...
- **WHEN** user runs `superpowers schema init my-workflow --description "My workflow" --artifacts proposal,tasks`
...
- **WHEN** user runs `superpowers schema init my-workflow` in interactive mode
- **AND** user confirms setting as default
- **THEN** system updates `superpowers/config.yaml` with `defaultSchema: my-workflow`

#### Scenario: Set as default via flag
- **WHEN** user runs `superpowers schema init my-workflow --default`
- **THEN** system creates schema and updates `superpowers/config.yaml` with `defaultSchema: my-workflow`

#### Scenario: Skip setting default
- **WHEN** user runs `superpowers schema init my-workflow --no-default`
- **THEN** system creates schema without modifying `superpowers/config.yaml`
...
- **WHEN** user runs `superpowers schema init my-workflow --json --description "Test" --artifacts proposal`
