...
## Purpose
Provide programmatic utilities for creating and validating Superpowers change directories.
## Requirements
...
- **THEN** the system creates `superpowers/changes/add-auth/` directory

#### Scenario: Duplicate change rejected
- **WHEN** `createChange(projectRoot, 'add-auth')` is called and `superpowers/changes/add-auth/` already exists
- **THEN** the system throws an error indicating the change already exists

#### Scenario: Creates parent directories if needed
- **WHEN** `createChange(projectRoot, 'add-auth')` is called and `superpowers/changes/` does not exist
