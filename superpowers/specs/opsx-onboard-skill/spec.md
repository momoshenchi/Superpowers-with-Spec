...
- **THEN** agent briefly demonstrates `/sp:explore` by investigating relevant code
...
- **THEN** agent runs `superpowers new change "<name>"` with derived kebab-case name
...
- **AND** provides command reference table (/sp:explore, /sp:new, /sp:ff, /sp:continue, /sp:apply, /sp:verify, /sp:archive)
- **AND** suggests next actions (try /sp:new or /sp:ff on something)
...
- **AND** explains how to continue later with `/sp:continue <name>`
