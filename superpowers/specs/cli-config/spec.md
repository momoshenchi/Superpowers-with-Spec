...
## Purpose
Provide a user-friendly CLI interface for viewing and modifying global Superpowers configuration settings without manually editing JSON files.
## Requirements
...
- **WHEN** user executes `superpowers config --help`
...
- **WHEN** user executes `superpowers config path`
...
- **WHEN** user executes `superpowers config get <key>` with a valid top-level key
- **THEN** print the raw value only (no labels or formatting)
- **AND** exit with code 0

#### Scenario: Get nested key with dot notation

- **WHEN** user executes `superpowers config get featureFlags.someFlag`
- **THEN** traverse the nested structure using dot notation
- **AND** print the value at that path

#### Scenario: Get non-existent key

- **WHEN** user executes `superpowers config get <key>` with a key that does not exist
- **THEN** print nothing (empty output)
- **AND** exit with code 1

#### Scenario: Get object value

- **WHEN** user executes `superpowers config get <key>` where the value is an object
...
- **WHEN** user executes `superpowers config set <key> <value>`
...
- **WHEN** user executes `superpowers config set <key> true` or `superpowers config set <key> false`
...
- **WHEN** user executes `superpowers config set <key> <value> --string`
...
- **WHEN** user executes `superpowers config set featureFlags.newFlag true`
...
- **WHEN** user executes `superpowers config unset <key>`
...
- **WHEN** user executes `superpowers config reset --all`
...
- **WHEN** user executes `superpowers config reset --all -y`
- **THEN** reset without prompting for confirmation

#### Scenario: Reset without --all flag

- **WHEN** user executes `superpowers config reset` without `--all`
...
- **WHEN** user executes `superpowers config edit`
...
The `superpowers config profile` command SHALL provide an action-first interactive flow that allows users to modify delivery and workflow settings independently.

#### Scenario: Current profile summary appears first

- **WHEN** user runs `superpowers config profile` in an interactive terminal
...
- **WHEN** user runs `superpowers config profile` interactively
...
- **WHEN** delivery selection is shown in `superpowers config profile`
...
- **WHEN** `superpowers config profile` exits with `No config changes.` inside an Superpowers project
...
- **AND** include guidance to run `superpowers update` to sync project files

#### Scenario: Apply prompt is gated on actual changes

- **WHEN** config values were changed and saved
- **AND** current directory is an Superpowers project
- **THEN** prompt `Apply changes to this project now?`
- **AND** if confirmed, run `superpowers update` for the current project
...
- **WHEN** user executes `superpowers config set someFutureKey 123`
...
- **WHEN** user executes `superpowers config set someFutureKey 123 --allow-unknown`
- **THEN** the value is saved successfully
- **AND** exit with code 0

#### Scenario: Invalid feature flag value rejected

- **WHEN** user executes `superpowers config set featureFlags.someFlag notABoolean`
...
- **WHEN** user executes `superpowers config --scope project <subcommand>`
