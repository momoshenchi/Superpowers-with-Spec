...
## Purpose
Define `superpowers change` command behavior for showing, listing, and validating change proposals and deltas.
...
- **WHEN** executing `superpowers change show update-error --json`
...
- **WHEN** executing `superpowers change list`
- **THEN** scan the superpowers/changes directory
- **AND** return list of all pending changes
- **AND** support JSON output with `--json` flag

#### Scenario: Show only requirement changes

- **WHEN** executing `superpowers change show update-error --requirements-only`
...
- **WHEN** executing `superpowers change validate update-error`
...
- **WHEN** executing `superpowers list`
- **THEN** display current list of changes (existing behavior)
- **AND** show deprecation notice: "Note: 'superpowers list' is deprecated. Use 'superpowers change list' instead."

#### Scenario: Legacy list with --all flag

- **WHEN** executing `superpowers list --all`
...
- **WHEN** executing `superpowers change show` without arguments
...
- **WHEN** executing `superpowers change show` without a change name
...
- **WHEN** executing `superpowers change validate` without arguments
- **THEN** display an interactive list of available changes
...
- **GIVEN** stdin is not a TTY or `--no-interactive` is provided or environment variable `OPEN_SPEC_INTERACTIVE=0`
- **WHEN** executing `superpowers change validate` without a change name
- **THEN** do not prompt interactively
- **AND** print the existing hint including available change IDs
- **AND** set `process.exitCode = 1`
