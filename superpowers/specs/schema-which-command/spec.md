...
## Purpose
Define `superpowers schema which` behavior for reporting resolved schema source, location, and fallback details.

## Requirements
### Requirement: Schema which shows resolution result
The CLI SHALL provide an `superpowers schema which <name>` command that displays where a schema resolves from.

#### Scenario: Schema resolves from project
- **WHEN** user runs `superpowers schema which my-workflow` and schema exists in `superpowers/schemas/my-workflow/`
...
- **WHEN** user runs `superpowers schema which my-workflow` and schema exists only in user data directory
...
- **WHEN** user runs `superpowers schema which spec-driven` and no override exists
- **THEN** system displays source as "package"
- **AND** displays full path to package's schemas directory

#### Scenario: Schema not found
- **WHEN** user runs `superpowers schema which nonexistent`
...
- **WHEN** user runs `superpowers schema which spec-driven` and both project and package have `spec-driven`
...
- **WHEN** user runs `superpowers schema which spec-driven --json`
- **THEN** system outputs JSON with `name`, `source`, and `path` fields

#### Scenario: JSON output with shadows
- **WHEN** user runs `superpowers schema which spec-driven --json` and schema has shadows
...
- **WHEN** user runs `superpowers schema which --all`
...
- **WHEN** user runs `superpowers schema which --all --json`
