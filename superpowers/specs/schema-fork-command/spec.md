...
## Purpose
Define `superpowers schema fork` behavior for cloning existing schemas into project-local schemas with safe overwrite controls.

## Requirements
### Requirement: Schema fork copies existing schema
The CLI SHALL provide an `superpowers schema fork <source> [name]` command that copies an existing schema to the project's `superpowers/schemas/` directory.

#### Scenario: Fork with explicit name
- **WHEN** user runs `superpowers schema fork spec-driven my-custom`
- **THEN** system locates `spec-driven` schema using resolution order (project → user → package)
- **AND** copies all files to `superpowers/schemas/my-custom/`
...
- **WHEN** user runs `superpowers schema fork spec-driven` without specifying a name
- **THEN** system copies to `superpowers/schemas/spec-driven-custom/`
- **AND** updates `name` field in `schema.yaml` to `spec-driven-custom`

#### Scenario: Source schema not found
- **WHEN** user runs `superpowers schema fork nonexistent`
...
- **WHEN** user runs `superpowers schema fork spec-driven my-custom` and `superpowers/schemas/my-custom/` exists
...
- **WHEN** user runs `superpowers schema fork spec-driven my-custom --force` and destination exists
...
- **WHEN** user runs `superpowers schema fork spec-driven my-custom` in interactive mode and destination exists
...
- **WHEN** user runs `superpowers schema fork spec-driven my-custom --json`
...
- **WHEN** user runs `superpowers schema fork spec-driven --json`
