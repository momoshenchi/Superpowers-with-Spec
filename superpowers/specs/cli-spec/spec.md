...
## Purpose
Define `superpowers spec` command behavior for listing, showing, and validating source-of-truth specifications.
...
- **WHEN** executing `superpowers spec show` without arguments
...
- **WHEN** executing `superpowers spec show` without a spec-id
...
- **WHEN** executing `superpowers spec show init --json`
...
- **WHEN** executing `superpowers spec validate` without arguments
- **THEN** display an interactive list of available specs
...
- **GIVEN** stdin is not a TTY or `--no-interactive` is provided or environment variable `OPEN_SPEC_INTERACTIVE=0`
- **WHEN** executing `superpowers spec validate` without a spec-id
- **THEN** do not prompt interactively
- **AND** print the existing error message for missing spec-id
- **AND** set non-zero exit code
