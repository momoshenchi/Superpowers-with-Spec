...
- **WHEN** user runs `superpowers status --change <id>`
...
- **WHEN** user runs `superpowers status --change <id>` on a change with no artifacts
...
- **WHEN** user runs `superpowers status` without `--change`
...
- **WHEN** user runs `superpowers status --change unknown-id`
- **AND** directory `superpowers/changes/unknown-id/` does not exist
- **THEN** the system displays an error listing all available change directories

### Requirement: Next Artifact Discovery

The workflow SHALL use `superpowers status` output to determine what can be created next, rather than a separate next-command surface.

#### Scenario: Discover next artifacts from status output

- **WHEN** a user needs to know which artifact to create next
- **THEN** `superpowers status --change <id>` identifies ready artifacts with `[ ]`
...
- **WHEN** user runs `superpowers templates`
...
- **WHEN** user runs `superpowers new change add-feature`
- **THEN** the system creates `superpowers/changes/add-feature/` directory

#### Scenario: Invalid change name
- **WHEN** user runs `superpowers new change "Add Feature"` with invalid name
- **THEN** the system displays validation error with guidance

#### Scenario: Duplicate change name
- **WHEN** user runs `superpowers new change existing-change` for an existing change
- **THEN** the system displays an error indicating the change already exists

#### Scenario: Create with description
- **WHEN** user runs `superpowers new change add-feature --description "Add new feature"`
...
- **WHEN** user runs `superpowers status --change <id> --schema tdd`
...
- **WHEN** user runs `superpowers artifact-experimental-setup --tool cursor`
...
- **WHEN** user runs `superpowers artifact-experimental-setup` without `--tool`
...
- **WHEN** user runs `superpowers artifact-experimental-setup --tool unknown-tool`
