...
## Purpose
Define top-level `superpowers show` behavior for interactive and direct display of change and spec content.
...
- **WHEN** executing `superpowers show` without arguments
...
- **WHEN** executing `superpowers show` without arguments
- **THEN** do not prompt
- **AND** print a helpful hint with examples for `superpowers show <item>` or `superpowers change/spec show`
- **AND** exit with code 1

#### Scenario: Direct item display

- **WHEN** executing `superpowers show <item-name>`
...
- **AND** if it matches both, print an ambiguity error and suggest `--type change|spec` or using `superpowers change show`/`superpowers spec show`
- **AND** if it matches neither, print not-found with nearest-match suggestions

#### Scenario: Explicit type override

- **WHEN** executing `superpowers show --type change <item>`
- **THEN** treat `<item>` as a change ID and show it (skipping auto-detection)

- **WHEN** executing `superpowers show --type spec <item>`
...
- **WHEN** executing `superpowers show <item> --json`
...
- **WHEN** showing a change with `superpowers show <change-name> --deltas-only`
...
- **WHEN** showing a spec with `superpowers show <spec-id> --requirements`
