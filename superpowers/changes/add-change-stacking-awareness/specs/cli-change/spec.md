...
- **WHEN** a user runs `superpowers change graph`
...
- **WHEN** a user runs `superpowers change next`
...
- **WHEN** a user runs `superpowers change split <change-id>`
...
- **WHEN** a user runs `superpowers change split <change-id>` for a parent whose generated child directories already exist
