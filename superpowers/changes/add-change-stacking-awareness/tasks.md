...
- [ ] 3.1 Add `superpowers change graph` to display dependency order for active changes
- [ ] 3.2 Add `superpowers change next` to suggest unblocked changes in recommended order
- [ ] 3.3 Add tests for topological ordering and deterministic tie-breaking (lexicographic by change ID at equal depth)

## 4. Split Scaffolding

- [ ] 4.1 Add `superpowers change split <change-id>` to scaffold child slices
...
- [ ] 4.5 Implement and test explicit overwrite mode for `superpowers change split` (`--overwrite` / `--force`) for controlled re-splitting
...
- [ ] 5.4 Document migration guidance for `superpowers/changes/IMPLEMENTATION_ORDER.md` as optional narrative, not dependency source of truth
