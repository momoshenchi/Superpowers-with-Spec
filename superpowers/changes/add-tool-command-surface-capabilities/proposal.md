...
Superpowers currently assumes command delivery maps directly to command adapters. That assumption does not hold for all tools.

Trae is a concrete example: it invokes Superpowers workflows via skill entries (for example `/superpowers-new-change`) rather than adapter-generated command files. In this model, skills are the command surface.
...
- `none`: no Superpowers command surface
