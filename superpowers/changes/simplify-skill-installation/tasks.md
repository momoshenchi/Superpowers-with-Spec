...
- [x] 3.6 Display post-update message: "Config updated. Run `superpowers update` in your projects to apply."
- [x] 3.7 Detect if running inside an Superpowers project and offer to run update automatically
...
- [x] 3.10 Update `superpowers config list` to display profile, delivery, and workflows settings (indicate defaults vs explicit)
...
- [x] 5.3 Ensure propose creates `.superpowers.yaml` via `superpowers new change` before generating artifacts
...
- [x] 5.7 Add `superpowers-propose` to `SKILL_NAMES` in `src/core/shared/tool-detection.ts`
...
- [x] 7.6 Update success message to show `/sp:propose` prompt (only if propose is in the active profile)
...
- [x] 8.4 Display migration message: "Migrated: custom profile with N workflows" + "New in this version: /sp:propose. Try 'superpowers config profile core' for the streamlined experience."
- [x] 8.5 Add project check: exit with error if no `superpowers/` directory exists
...
- [x] 10.1 Verify `superpowers new change` creates `.superpowers.yaml` with schema and created fields

<!-- Note: 10.2 and 10.3 below are potential follow-up work, not core to this change -->
<!-- - [ ] 10.2 Update ff skill to verify `.superpowers.yaml` exists after `superpowers new change` -->
<!-- - [ ] 10.3 Add guardrail to skills: "Never manually create files in superpowers/changes/ - use superpowers new change" -->

## 11. Template Next-Step Guidance

- [x] 11.1 Audit all templates for hardcoded cross-workflow command references (e.g., `/sp:propose`)
- [x] 11.2 Replace any specific command references with generic concept-based guidance (e.g., "create a change proposal")
- [x] 11.3 Review explore → propose transition UX (see `superpowers/explorations/explore-workflow-ux.md` for open questions)
...
- [x] 12.6 Manual: `superpowers config profile` picker — verify delivery toggle, workflow toggles, pre-selection of current values, core preset shortcut
