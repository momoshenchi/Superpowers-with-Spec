...
Superpowers currently installs 10 workflows (skills + commands) for every user, overwhelming new users. The init flow asks multiple questions (profile, delivery, tools) creating friction before users can experience value.
...
Global config exists at `~/.config/superpowers/config.json` for telemetry/feature flags. Profile/delivery settings will extend this existing config.
...
Add profile/delivery settings to existing `~/.config/superpowers/config.json` (via `src/core/global-config.ts`).
...
- New `~/.superpowers/config.yaml`: Creates second config file, different format, path confusion
...
1. Creates change directory via `superpowers new change`
...
**Rationale:** Most users don't know or care about this distinction. Power users who have a preference can set it via `superpowers config profile` interactive picker. Not worth asking during init.
...
**Behavior:**
- `superpowers init` sets up new projects OR re-initializes existing projects (selects tools, generates workflows)
- `superpowers update` refreshes an existing project to match current config (no tool selection)
- `superpowers config profile` updates global config only, offers to run update if in a project
...
- *Gitignored local file* (`.superpowers.local`): Lost on fresh clone, adds file management overhead.
- *Checked-in project config* (`superpowers/config.yaml` with `tools` field): Forces tool choices on the whole team — Alice uses Claude Code, Bob uses Cursor, neither wants the other's tools mandated.

The filesystem approach avoids all three problems. For teams, it's actually beneficial: checked-in skill files mean `superpowers update` from any team member refreshes skills for all tools the project supports. The generated files serve as both the deliverable and the implicit tool manifest.
...
When `superpowers init` or `superpowers update` encounters a project with existing workflows but no `profile` field in global config, it performs a one-time migration to preserve the user's current setup.
...
- Map directory names back to workflow IDs (e.g., `superpowers-explore/` → `explore`, `sp-explore.md` → `explore`)
...
- **User has custom (non-Superpowers) skills in the directory:** Ignored — scanner only matches known workflow IDs from `ALL_WORKFLOWS`
...
Workflow templates use generic, concept-based next-step guidance rather than referencing specific workflow commands. For example, instead of "run `/sp:propose`", templates say "create a change proposal".
...
`superpowers update` cannot rely only on `generatedBy` version checks for deciding whether work is needed.
...
`superpowers init --profile` must validate allowed values before proceeding.
...
→ Acceptable: These cover the main loop. Users who need more can use `superpowers config profile` to select additional workflows.
...
4. **Phase 4: Add config profile command**
   - `superpowers config profile` interactive picker
   - `superpowers config profile core` preset shortcut
