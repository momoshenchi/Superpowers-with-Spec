...
- **WHEN** user runs `superpowers init` interactively and tool directories are detected
...
- **WHEN** user runs `superpowers init` interactively and no tool directories are detected
- **THEN** the system SHALL prompt for tool selection
- **THEN** the system SHALL use default profile (`core`) and delivery (`both`)

#### Scenario: Non-interactive with detected tools
- **WHEN** user runs `superpowers init` non-interactively (e.g., in CI)
...
- **WHEN** user runs `superpowers init` non-interactively
...
- **WHEN** user runs `superpowers init --tools claude`
...
- **WHEN** user runs `superpowers init --tools claude` interactively
...
- **THEN** for tools using colon syntax (Claude Code): "Start your first change: /sp:propose \"your idea\""
- **THEN** for tools using hyphen syntax (Cursor, others): "Start your first change: /sp-propose \"your idea\""
...
- **THEN** for tools using colon syntax: "Start your first change: /sp:new \"your idea\""
- **THEN** for tools using hyphen syntax: "Start your first change: /sp-new \"your idea\""

#### Scenario: Init success message (neither propose nor new)
- **WHEN** init completes successfully
- **AND** neither `propose` nor `new` is in the active profile
- **THEN** the system SHALL display: "Done. Run 'superpowers config profile' to configure your workflows."
...
- **WHEN** user runs `superpowers init` on a project with existing workflow files
...
- **WHEN** user runs `superpowers init` on a project with no existing workflow files
- **AND** global config does not contain a `profile` field
...
- **WHEN** user runs `superpowers init --profile core`
...
- **WHEN** user runs `superpowers init --profile <invalid>`
...
- **WHEN** user runs `superpowers init` interactively
...
- **WHEN** user has custom profile with extra workflows and runs `superpowers init` with core profile
...
- **WHEN** user runs `superpowers init` on existing project
...
- **WHEN** user runs `superpowers init` on an existing project
