...
- **WHEN** user runs `superpowers update`
...
- **WHEN** user runs `superpowers update`
- **AND** a tool has generated Superpowers command files
- **AND** that tool has no Superpowers skill files (commands-only delivery)
...
- **THEN** the system SHALL display: "New in this version: /sp:propose (combines new + ff). Try 'superpowers config profile core' for the streamlined 4-workflow experience."
...
- **WHEN** user runs `superpowers update` multiple times
...
- **WHEN** user runs `superpowers update` non-interactively (e.g., in CI)
...
- **THEN** the system SHALL display: "Detected new tool: Windsurf. Run 'superpowers init' to add it."
...
- **THEN** the system SHALL display one consolidated message listing all detected tools, for example: "Detected new tools: GitHub Copilot, Windsurf. Run 'superpowers init' to add them."
...
### Requirement: Update requires an Superpowers project
The update command SHALL only run inside an initialized Superpowers project.

#### Scenario: Update outside a project
- **WHEN** user runs `superpowers update`
- **AND** no `superpowers/` directory exists in the current working directory
- **THEN** the system SHALL display: "No Superpowers project found. Run 'superpowers init' to set up."
...
- **WHEN** user runs `superpowers update`
- **AND** project has workflows not in current profile (e.g., user switched from custom to core or deselected workflows via `superpowers config profile`)
