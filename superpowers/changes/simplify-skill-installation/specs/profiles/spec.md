...
- **WHEN** user runs `superpowers config profile`
...
- **THEN** the system SHALL display: "Config updated. Run `superpowers update` in your projects to apply."

#### Scenario: Core preset shortcut
- **WHEN** user runs `superpowers config profile core`
...
- **THEN** the system SHALL NOT modify any project files
- **THEN** the system SHALL display: "Config updated. Run `superpowers update` in your projects to apply."
- **THEN** the new profile takes effect on the next `superpowers init` or `superpowers update` run

#### Scenario: Config profile run inside a project
- **WHEN** user runs `superpowers config profile` inside an Superpowers project directory
- **THEN** after updating global config, the system SHALL prompt: "Apply to this project now? (y/n)"
- **WHEN** user confirms
- **THEN** the system SHALL run `superpowers update` automatically
- **THEN** the system SHALL still display: "Run `superpowers update` in your other projects to apply."
...
- **WHEN** user runs `superpowers config profile` non-interactively (e.g., in CI, no TTY)
- **THEN** the system SHALL display an error: "Interactive mode required. Use `superpowers config profile core` or set config via environment/flags."
- **THEN** the system SHALL exit with code 1

### Requirement: Profile settings stored in global config
Profile and delivery settings SHALL be stored in the existing global config file (`~/.config/superpowers/config.json`) alongside telemetry and feature flags.
...
- **WHEN** user runs `superpowers config list`
...
- **WHEN** user updates config via `superpowers config profile`
- **THEN** the system SHALL only update global config (`~/.config/superpowers/config.json`)
- **THEN** the system SHALL NOT modify any project skill/command files
- **THEN** existing projects retain their current workflow files until user runs `superpowers update`

### Requirement: Config changes applied via update command
The existing `superpowers update` command SHALL apply the current global config to a project. See `specs/cli-update/spec.md` for detailed update behavior.

#### Scenario: Config changes require explicit project sync
- **WHEN** user updates profile or delivery via `superpowers config profile`
- **THEN** the global config SHALL be updated immediately
- **AND** project files SHALL remain unchanged until `superpowers update` is run for that project
