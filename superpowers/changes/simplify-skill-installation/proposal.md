...
```text
0:00  $ superpowers init
      ✓ Done. Run /sp:propose "your idea"

0:15  /sp:propose "add user authentication"
...
Power users can customize via `superpowers config profile`.
...
`/sp:propose` should naturally onboard users by explaining what it's doing:
...
Stored in existing global config (`~/.config/superpowers/config.json`). Not prompted during init.
...
superpowers config profile core     # preset shortcut (core workflows, preserves delivery)
```
...
**Existing users keep their current setup.** When `superpowers update` runs on a project with existing workflows and no `profile` in global config, it performs a one-time migration:
...
- `superpowers init` on a **new** project (no existing workflows) uses global config, defaulting to `core`
...
- Migration message mentions `propose` and suggests `superpowers config profile core` to opt in
- After migration, users can opt into `core` profile via `superpowers config profile core`
...
```json
// ~/.config/superpowers/config.json (extends existing)
{
  "telemetry": { ... },          // existing
...
| custom | user-defined | Pick exactly what you need via `superpowers config profile` |
