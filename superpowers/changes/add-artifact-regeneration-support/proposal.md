...
1. Delete tasks.md manually, then run `/sp:continue`
...
Add artifact staleness detection to `/sp:apply`:

1. **Track modification times**: When generating an artifact, record the mtime of its dependencies
2. **Detect staleness**: When `/sp:apply` runs, check if upstream artifacts (design.md, specs) have been modified since tasks.md was generated
3. **Prompt user**: If stale, ask: "Design was modified after tasks were generated. Would you like to regenerate tasks with `/sp:continue`?"
...
Store `.superpowers-meta.json` in change directory:
```json
{
  "tasks.md": {
