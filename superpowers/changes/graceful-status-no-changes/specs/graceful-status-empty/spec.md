...
- **WHEN** user runs `superpowers status` without `--change` and no change directories exist under `superpowers/changes/`
- **THEN** the CLI prints `No active changes. Create one with: superpowers new change <name>` to stdout and exits with code 0

#### Scenario: No changes exist, JSON mode
- **WHEN** user runs `superpowers status --json` without `--change` and no change directories exist
...
- **WHEN** user runs `superpowers status` without `--change` and one or more change directories exist
...
- **WHEN** user runs `superpowers status --change non-existent`
- **THEN** the CLI throws an error with message `Change 'non-existent' not found`

#### Scenario: Other commands unaffected
- **WHEN** user runs `superpowers show` or `superpowers instructions` without `--change` and no changes exist
