...
- `superpowers status` will exit gracefully (code 0) with a friendly message when no changes exist, instead of throwing a fatal error
- `superpowers status --json` will return a valid JSON object with an empty changes array when no changes exist
...
- `graceful-status-empty`: Graceful handling of `superpowers status` when no changes exist, covering both text and JSON output modes
