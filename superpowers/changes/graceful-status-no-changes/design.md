...
The error surfaces during onboarding (issue #714) when AI agents call `superpowers status` before any change has been created.

## Goals / Non-Goals

**Goals:**
- Make `superpowers status` exit with code 0 and a friendly message when no changes exist
