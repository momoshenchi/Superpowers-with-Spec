...
Superpowers installation paths are currently inconsistent:
...
- Add install scope control in `superpowers config profile` interactive flow.
- Extend `superpowers config list` output with install scope source (`explicit`, `new-default`, `legacy-default`).
