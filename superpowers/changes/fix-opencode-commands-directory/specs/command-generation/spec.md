...
- **AND** file path SHALL follow pattern `.claude/commands/sp/<id>.md`

#### Scenario: Cursor adapter formatting

- **WHEN** formatting a command for Cursor
- **THEN** the adapter SHALL output YAML frontmatter with `name` as `/sp-<id>`, `id`, `category`, `description` fields
- **AND** file path SHALL follow pattern `.cursor/commands/sp-<id>.md`

#### Scenario: Windsurf adapter formatting

- **WHEN** formatting a command for Windsurf
- **THEN** the adapter SHALL output YAML frontmatter with `name`, `description`, `category`, `tags` fields
- **AND** file path SHALL follow pattern `.windsurf/workflows/sp-<id>.md`
...
- **AND** file path SHALL follow pattern `.opencode/commands/sp-<id>.md` using `path.join('.opencode', 'commands', ...)` for cross-platform compatibility
- **AND** the adapter SHALL transform colon-based command references (`/sp:name`) to hyphen-based (`/sp-name`) in the body
...
- **WHEN** running legacy artifact detection on a project with files matching `.opencode/command/sp-*.md` or `.opencode/command/superpowers-*.md`
...
- **WHEN** a user runs `superpowers init` in a project with old `.opencode/command/` artifacts
...
- **WHEN** a user runs `superpowers init` in non-interactive mode (e.g., CI) and legacy artifacts are detected
- **THEN** the system SHALL auto-cleanup legacy artifacts without requiring `--force`
- **AND** legacy slash command files (100% Superpowers-managed) SHALL be removed
- **AND** config file cleanup SHALL only remove Superpowers markers (never delete user files)
