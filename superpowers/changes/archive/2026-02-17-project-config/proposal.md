...
Add `superpowers/config.yaml` support for project-level configuration. This enables teams to customize Superpowers behavior without forking schemas, by providing context and rules that are injected into artifact generation.

## Motivation

Currently, customizing Superpowers requires forking entire schemas:
- Must copy all files even to add one rule
- Lose updates when superpowers upgrades
...
Superpowers customization follows two distinct paths:
...
│   + add rules                                                   │
│                                                                 │
│   superpowers/config.yaml            superpowers/schemas/my-flow/     │
│                                                                 │
│   ✓ Simple                        ✓ Full control                │
...
- Commands run on changes without `.superpowers.yaml` metadata
...
3. **Avoids premature API surface** - No standalone `superpowers config init` until feature graduates
...
```
Today (Experimental):
  superpowers artifact-experimental-setup
    → prompts for config creation
    → creates .claude/skills/
    → creates superpowers/config.yaml
...
Future (When graduating):
  ...
  + standalone commands:
    ...
    superpowers config validate
    superpowers config set <key> <value>
```

**Why optional?**

Config is **additive**, not required:
- Superpowers works without config (uses defaults)
...
- Update `superpowers new change` to use config's schema as default
...
- Integration with standard `superpowers init` (will add when experimental graduates)
- Standalone `superpowers config init` command (may add in future change)
- `superpowers config validate` command (may add in future change)
...
✓ Created .claude/skills/superpowers-explore/SKILL.md
✓ Created .claude/skills/superpowers-new-change/SKILL.md
✓ Created .claude/skills/superpowers-continue-change/SKILL.md
✓ Created .claude/skills/superpowers-apply-change/SKILL.md
✓ Created .claude/skills/superpowers-ff-change/SKILL.md
✓ Created .claude/skills/superpowers-sync-specs/SKILL.md
✓ Created .claude/skills/superpowers-archive-change/SKILL.md

✓ Created .claude/commands/sp/explore.md
✓ Created .claude/commands/sp/new.md
✓ Created .claude/commands/sp/continue.md
✓ Created .claude/commands/sp/apply.md
✓ Created .claude/commands/sp/ff.md
✓ Created .claude/commands/sp/sync.md
✓ Created .claude/commands/sp/archive.md

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

📋 Project Configuration (Optional)

Configure project defaults for Superpowers workflows.

? Create superpowers/config.yaml? (Y/n) Y
...
✓ Created superpowers/config.yaml

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

🎉 Setup Complete!

📖 Config created at: superpowers/config.yaml
...
To share with team:
  git add superpowers/config.yaml .claude/
  git commit -m "Setup Superpowers experimental workflow with project config"
...
/sp:new my-feature --schema spec-driven
...
superpowers instructions proposal --change my-feature
...
superpowers instructions specs --change my-feature
...
```bash
superpowers instructions design --change my-feature
...
```bash
# Commit config
git add superpowers/config.yaml
...
Always at `./superpowers/config.yaml` relative to project root. No XDG/global config for simplicity.
...
2. .superpowers.yaml in change directory   # Change-specific binding
3. superpowers/config.yaml schema field    # Project default (NEW)
...
6. **Create and validate config:**
   ...
   - Write to `superpowers/config.yaml` using YAML serializer
...
When `superpowers/config.yaml` already exists:
...
ℹ️  superpowers/config.yaml already exists. Skipping config creation.

   To update config, edit superpowers/config.yaml manually or:
   1. Delete superpowers/config.yaml
   2. Run superpowers artifact-experimental-setup again
...
try {
  ...
  if (configResult.createConfig) {
    ...
    console.log('✓ Created superpowers/config.yaml');
  }
} catch (error) {
...
1. **@inquirer/prompts** (recommended)
   ...
   - Already used in Superpowers (if not, lightweight addition)
...
- Verify change's `.superpowers.yaml` overrides config
...
- Test all four levels of schema resolution:
  ...
  2. Change metadata `.superpowers.yaml`
...
- Existing changes with `.superpowers.yaml` metadata aren't affected by config
