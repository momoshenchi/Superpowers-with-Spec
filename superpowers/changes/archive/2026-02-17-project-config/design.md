...
Superpowers currently has a fixed schema resolution order:
1. `--schema` CLI flag
2. `.superpowers.yaml` in change directory
...
The proposal introduces `superpowers/config.yaml` as a lightweight customization layer that sits between preset schemas and full forking. It allows teams to:
...
**Key stakeholders:**
- Superpowers users who need light customization without forking
- Teams sharing workflow conventions via committed config

## Goals / Non-Goals

**Goals:**
- Load and parse `superpowers/config.yaml` using Zod schema
...
- Config management commands (`superpowers config init`) - manual creation for now
...
- Consistent with `.superpowers.yaml` used in changes
...
### 2. Config Location: Project Root vs superpowers/ Directory

**Decision:** `./superpowers/config.yaml` (inside superpowers directory)

**Rationale:**
- Co-located with `superpowers/schemas/` (project-local-schemas)
- Keeps project root clean
- Natural namespace for Superpowers configuration
- Mirrors structure used by other tools (e.g., `.github/`)

**Alternatives considered:**
- `./superpowers.config.yaml` in root: Pollutes root, less clear ownership
...
3. **`superpowers/config.yaml` schema field** (NEW - project default)
...
**Rationale:**
- Syntax errors in config shouldn't break all of Superpowers
- User can fix config incrementally
- Commands remain usable during config development

**Warning message:**
```
⚠️  Failed to parse superpowers/config.yaml: [error details]
 ...
 * - Helps understand Superpowers's data interfaces at a glance
 * - Single source of truth for type and validation
 * - Consistent with other Superpowers schemas
 */
...
/**
 * Read and parse superpowers/config.yaml from project root.
...
export function readProjectConfig(): ProjectConfig | null {
  ...
  let configPath = path.join(projectRoot, 'superpowers', 'config.yaml');
  if (!existsSync(configPath)) {
    configPath = path.join(projectRoot, 'superpowers', 'config.yml');
    if (!existsSync(configPath)) {
  ...
  try {
    ...
    if (!raw || typeof raw !== 'object') {
      console.warn(`⚠️  superpowers/config.yaml is not a valid YAML object`);
      return null;
    }
    ...
    console.warn(`⚠️  Failed to parse superpowers/config.yaml:`, error);
    return null;
  }
}
...
export function suggestSchemas(
  ...
  let message = `❌ Schema '${invalidSchemaName}' not found in superpowers/config.yaml\n\n`;
  ...
  message += `\nFix: Edit superpowers/config.yaml and change 'schema: ${invalidSchemaName}' to a valid schema name`;
...
```typescript
// Example: "superpowers instructions proposal --change my-feature"
...
export function readProjectConfig(): ProjectConfig | null {
  const projectRoot = findProjectRoot();
  const configPath = path.join(projectRoot, 'superpowers', 'config.yaml');
...
┌──────────────────────────────────────────────────────────────┐
│                                                              │
│  User runs: superpowers instructions proposal --change foo     │
│                                                              │
└────────────────────────────┬─────────────────────────────────┘
                             │
                             ▼
...
│                                                              │
│  1. Check CLI flag ✗                                         │
│  2. Check .superpowers.yaml ✗                                   │
│  3. Check superpowers/config.yaml ✓ → "spec-driven"             │
│                                                              │
└────────────────────────────┬─────────────────────────────────┘
                             │
                             ▼
...
│  2. Read superpowers/config.yaml                                │
...
**[Risk]** YAML parsing errors break Superpowers commands
...
→ **Mitigation:** Document common artifact IDs, show in `superpowers status` output
