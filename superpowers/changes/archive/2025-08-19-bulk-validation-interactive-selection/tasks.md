...
- [x] 2.4 When no `[spec-id]` provided and interactivity allowed, prompt to select from `superpowers/specs/*/spec.md` and validate the selected spec
...
- [x] 3.1 Add `validate` command in `src/cli/index.ts`
  ...
  - Usage: `superpowers validate [item-name]`
- [x] 3.2 Create `src/commands/validate.ts` implementing:
  ...
  - [x] 3.2.6 Bulk validation for `--all`, `--changes`, `--specs` (exclude `superpowers/changes/archive/`)
...
- [x] 8.3 Ensure existing specs under `superpowers/changes/bulk-validation-interactive-selection/specs/*` remain satisfied
...
- [x] AC-1: `superpowers change validate` interactive selection when no arg (TTY only; respects `--no-interactive`/env) — matches cli-change spec
- [x] AC-2: `superpowers spec validate` interactive selection when no arg (TTY only; respects `--no-interactive`/env) — matches cli-spec spec
- [x] AC-3: New `superpowers validate` supports interactive selection, bulk/filtered validation, JSON schema, progress, concurrency, exit codes — matches cli-validate spec
