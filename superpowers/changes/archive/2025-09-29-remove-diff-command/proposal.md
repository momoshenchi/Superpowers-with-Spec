...
The `superpowers diff` command adds unnecessary complexity to the Superpowers CLI for several reasons:

1. **Redundant functionality**: The `superpowers show` command already provides comprehensive visualization of changes through structured JSON output and markdown rendering
...
Remove the `superpowers diff` command entirely and guide users to more appropriate alternatives:

1. **For viewing change content**: Use `superpowers show <change-name>` which provides:
...
- **Better alignment**: Focuses on the core Superpowers workflow without redundant features

## Implementation

### Files to Remove
- `/src/core/diff.ts` - The entire diff command implementation
- `/superpowers/specs/cli-diff/spec.md` - The diff command specification
...
- `/superpowers/README.md` - Remove diff command references
- Various documentation files mentioning `superpowers diff`

### Migration Guide for Users

Users currently using `superpowers diff` should transition to:

```bash
# Before
superpowers diff add-feature

# After - view the change proposal
superpowers show add-feature

# After - view only the deltas
superpowers show add-feature --json --deltas-only

# After - use git for file comparisons
git diff superpowers/specs superpowers/changes/add-feature/specs
```
