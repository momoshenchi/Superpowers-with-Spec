...
Superpowers currently resolves schemas from two locations:
1. User override: `~/.local/share/superpowers/schemas/<name>/`
2. Package built-in: `<npm-package>/schemas/<name>/`

This change adds a third, highest-priority level: project-local schemas at `./superpowers/schemas/<name>/`.
...
- Schema management CLI commands (`superpowers schema copy/which/diff/reset`)
...
```
1. ./superpowers/schemas/<name>/              # Project-local (highest)
2. ~/.local/share/superpowers/schemas/<name>/ # User override
...
function getProjectSchemasDir(projectRoot: string): string {
  return path.join(projectRoot, 'superpowers', 'schemas');
}
```
...
A team could create `superpowers/schemas/spec-driven/` that shadows the built-in, causing confusion when someone expects default behavior.

**Mitigation:** The `superpowers schemas` command shows the source of each schema. Users can see `spec-driven (project)` vs `spec-driven (package)`.
