...
Superpowers has a complete skill and slash command generation system. Skills are defined in `src/core/templates/skill-templates.ts` as functions that return `SkillTemplate` objects (for Agent Skills) and `CommandTemplate` objects (for slash commands). These are registered in `src/core/shared/skill-generation.ts` and generated during `superpowers init` and `superpowers update`.
...
**Goals:**
- Add `/sp:onboard` skill that teaches the Superpowers workflow through guided practice
