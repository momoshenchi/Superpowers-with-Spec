...
Acceptance:
- `superpowers show` exists and prints a helpful hint in non-interactive contexts when no args.
...
- [x] Create `ShowCommand` with:
  ...
  - Non-interactive path when `!itemName`:
    - Print hint with examples:
      - `superpowers show <item>`
      - `superpowers change show`
      - `superpowers spec show`
    - Exit with code 1.
...
Acceptance:
- `superpowers show <change-id> --json --deltas-only` matches `superpowers change show <id> --json --deltas-only` output.
- `superpowers show <spec-id> --json --requirements` matches `superpowers spec show <id> --json --requirements` output.
...
- [x] `src/commands/change.ts` → extend `show(changeName?: string, options?: { json?: boolean; requirementsOnly?: boolean; deltasOnly?: boolean; noInteractive?: boolean })`:
  ...
  - Non-interactive fallback: keep current behavior (print available IDs + `superpowers change list` hint, set `process.exitCode = 1`).
...
Acceptance:
- `superpowers change show` in non-interactive prints list hint and exits non-zero.
- `superpowers spec show` in non-interactive prints missing-arg error and exits non-zero.
...
- [x] Top-level `show` hint (non-interactive no-arg):
  - Lines include: `superpowers show <item>`, `superpowers change show`, `superpowers spec show`, and "Or run in an interactive terminal.".
...
- [x] `test/commands/change.interactive-show.test.ts` (non-interactive fallback)
  - Ensure `superpowers change show` without args prints available IDs + list hint and non-zero exit.
- [x] `test/commands/spec.interactive-show.test.ts` (non-interactive fallback)
  - Ensure `superpowers spec show` without args prints missing-arg error and non-zero exit.
...
- [x] Update `superpowers/README.md` usage examples to include the new `show` command with type detection and flags.
