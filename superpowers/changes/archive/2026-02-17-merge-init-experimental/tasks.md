...
- [x] 1.2 Implement `detectLegacyConfigFiles()` - check for config files with Superpowers markers
- [x] 1.3 Implement `detectLegacySlashCommands()` - check for old `/superpowers:*` command directories
- [x] 1.4 Implement `detectLegacyStructureFiles()` - check for AGENTS.md (project.md detected separately for messaging)
- [x] 1.5 Implement `removeMarkerBlock()` - surgically remove Superpowers marker blocks from files
...
- [x] 2.8 Create `superpowers/config.yaml` with default schema
- [x] 2.9 Update success output to match new workflow (skills, /sp:* commands)
...
- [x] 5.1 Update `superpowers update` command to refresh skills/commands instead of config files
