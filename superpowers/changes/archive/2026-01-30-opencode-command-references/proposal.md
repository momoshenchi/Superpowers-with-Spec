...
OpenCode uses hyphen-based command syntax (`/sp-new`) but our templates contain colon-based references (`/sp:new`). This creates inconsistency where generated command files and skill files contain references that don't match the actual command invocation syntax, confusing both the AI and users.

## What Changes

- Create a shared transformation utility (`transformToHyphenCommands`) for converting `/sp:` to `/sp-`
...
- **Users**: OpenCode users will see correct `/sp-` command references in both generated command files AND skill files
