...
The templates in `src/core/templates/skill-templates.ts` use Claude's colon-based format (`/sp:new`) as the canonical format. Tools that use different formats need transformation at generation time.

## Goals / Non-Goals

**Goals:**
- Transform all `/sp:` command references to `/sp-` for OpenCode in both commands and skills
...
**Non-Goals:**
- Modifying the canonical template format (templates stay with `/sp:`)
...
| Risk | Mitigation |
|------|------------|
| Other `/sp:` patterns exist that shouldn't be transformed | All occurrences in templates are command invocations - verified by inspection |
