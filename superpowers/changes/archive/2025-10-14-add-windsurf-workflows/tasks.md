## 1. CLI wiring
- [x] 1.1 Add Windsurf to the selectable AI tools in `superpowers init`, including "already configured" detection.
...
- [x] 1.3 Ensure `superpowers update` pulls the Windsurf configurator when winds is selected and skips creation when files are absent.

## 2. Workflow templates
- [x] 2.1 Reuse the shared proposal/apply/archive bodies, adding Windsurf-specific headings/description before the Superpowers markers.
- [x] 2.2 Confirm generated Markdown (per file) stays comfortably under the 12k character ceiling noted in the Windsurf docs.

## 3. Tests & safeguards
- [x] 3.1 Extend init tests to assert creation of `.windsurf/workflows/superpowers-*.md` when Windsurf is chosen.
