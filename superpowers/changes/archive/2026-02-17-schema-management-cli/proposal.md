...
Add a new `superpowers schema` command group with subcommands for creating, forking, validating, and inspecting schemas.

### Commands

1. **`superpowers schema init <name>`** - Interactive wizard to scaffold a new project schema
   ...
   - Optionally sets as project default in `superpowers/config.yaml`

2. **`superpowers schema fork <source> [name]`** - Copy an existing schema as a starting point
...
3. **`superpowers schema validate [name]`** - Validate schema structure and templates
...
4. **`superpowers schema which <name>`** - Show schema resolution path
