...
- [x] 3.5 Add validate-all mode when no name provided (scan `superpowers/schemas/`)
- [x] 3.6 Add text output with pass/fail indicators and error messages
- [x] 3.7 Add JSON output matching existing `superpowers validate` format: `{ valid, issues: [] }`
...
- [x] 5.7 Add `--default` flag to update `superpowers/config.yaml` with new schema as default
