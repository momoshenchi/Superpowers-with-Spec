...
### 1. Command Structure: `superpowers schema <subcommand>`

Add a new command group following the existing pattern used by `superpowers config` and `superpowers completion`.
...
- Flat commands (`superpowers schema-init`, `superpowers schema-fork`): Rejected because it pollutes the top-level namespace and doesn't scale well.
- Extending existing commands (`superpowers init --schema`): Rejected because schema management is distinct from project initialization.
...
**Rationale:** Matches the UX established by `superpowers init` and `superpowers config reset`. Provides a guided experience while keeping the wizard lightweight.
...
The destination is always project-local: `superpowers/schemas/<name>/`
...
**Rationale:** Consistent with existing Superpowers commands and enables scripting/automation.
