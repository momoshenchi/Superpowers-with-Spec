## 1. Implementation
- [x] 1.1 Refactor `superpowers init` to always generate the root `AGENTS.md` stub (initial run and extend mode) via shared helper logic.
...
- [x] 2.2 Execute `superpowers validate update-cli-init-root-agents --strict`.
- [x] 2.3 Perform a manual smoke test: run `superpowers init` in a temp directory, confirm stub + grouped prompts, rerun in extend mode.
