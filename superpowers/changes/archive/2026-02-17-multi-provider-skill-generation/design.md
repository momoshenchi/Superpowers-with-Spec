...
The `artifact-experimental-setup` command generates skill files and sp slash commands for AI coding assistants. Currently it hardcodes paths to `.claude/skills` and `.claude/commands/sp`.

The existing `AI_TOOLS` array in `config.ts` lists 22 AI tools but lacks path information. There's also an existing `SlashCommandConfigurator` system for the old workflow commands, but it's tightly coupled to the old 3 commands (proposal, apply, archive) and can't be easily extended for the 9 sp commands.

Each AI tool has:
- Different skill directory conventions (`.claude/skills/`, `.cursor/skills/`, etc.)
- Different command file paths (`.claude/commands/sp/`, `.cursor/commands/`, etc.)
...
interface CommandContent {
  ...
  name: string;         // e.g., 'Superpowers Explore'
  description: string;  // e.g., 'Enter explore mode...'
  category: string;     // e.g., 'Superpowers'
  tags: string[];       // e.g., ['superpowers', 'explore']
