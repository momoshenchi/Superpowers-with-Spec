...
state via `process.cwd()` and `superpowers/changes`. This makes outcomes depend on
...
- Each suite that touches filesystem discovery creates a temporary directory:
  - `superpowers/changes/sample-change/proposal.md`
  - `superpowers/changes/sample-change/specs/sample/spec.md`
...
- Tests that previously depended on repo state now:
  ...
  - Do not read real `superpowers/changes` during execution.
