...
The `global-config` spec defines how Superpowers reads/writes `config.json`, but users currently must edit it by hand. This command provides a CLI interface to that config.
...
**Examples:**
```bash
superpowers config get featureFlags              # Returns object
superpowers config get featureFlags.experimental # Returns nested value
superpowers config set featureFlags.newFlag true
```
...
**Rationale:**
- Raw output enables piping: `VAR=$(superpowers config get key)`
