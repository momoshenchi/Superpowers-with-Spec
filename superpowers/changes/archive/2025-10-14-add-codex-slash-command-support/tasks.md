...
- [x] 1.2 Implement a `CodexSlashCommandConfigurator` that writes `.codex/prompts/superpowers-{proposal,apply,archive}.md`, ensuring the prompt directory exists and wrapping content in Superpowers markers.
...
- [x] 3.1 Update the `superpowers update` flow to refresh existing Codex prompts without creating new ones when files are missing.
...
- [x] 4.2 Add a release note snippet that points Codex users to the generated `/superpowers-proposal`, `/superpowers-apply`, and `/superpowers-archive` commands.
