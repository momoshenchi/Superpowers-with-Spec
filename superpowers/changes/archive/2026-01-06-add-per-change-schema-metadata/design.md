...
The metadata file (`.superpowers.yaml`) will be validated with this Zod schema:
...
/**
 * Schema for per-change metadata stored in .superpowers.yaml
 */
...
**Location:** `superpowers/changes/<name>/.superpowers.yaml`
...
- `superpowers.json` - YAML matches existing schema files
...
const METADATA_FILENAME = '.superpowers.yaml';
...
2. **`.superpowers.yaml` metadata** (persisted choice)
...
No migration needed:
- Existing changes without `.superpowers.yaml` continue to work (use default)
- New changes created with `superpowers new change --schema X` get metadata file

## Open Questions

- Should `superpowers new change` prompt for schema interactively if not specified? (Leaning no - default is fine)
