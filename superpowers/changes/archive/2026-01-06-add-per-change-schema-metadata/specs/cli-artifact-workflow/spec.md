...
The system SHALL store and validate per-change metadata in `.superpowers.yaml` files using a Zod schema.

#### Scenario: Metadata file created with new change

- **WHEN** user runs `superpowers new change add-feature --schema tdd`
- **THEN** the system creates `.superpowers.yaml` in the change directory
- **AND** the file contains `schema: tdd` and `created: <YYYY-MM-DD>`

#### Scenario: Metadata validated on read

- **WHEN** the system reads `.superpowers.yaml`
...
- **WHEN** `.superpowers.yaml` contains invalid YAML or missing required fields
- **THEN** the system displays a Zod validation error with details

#### Scenario: Missing metadata file

- **WHEN** a change directory has no `.superpowers.yaml` file
...
- **AND** creates `.superpowers.yaml` with `schema: spec-driven` (default)

#### Scenario: Create change with schema

- **WHEN** user runs `superpowers new change add-feature --schema tdd`
- **THEN** the system creates `superpowers/changes/add-feature/` directory
- **AND** creates `.superpowers.yaml` with `schema: tdd`

#### Scenario: Invalid schema on create

- **WHEN** user runs `superpowers new change add-feature --schema unknown`
...
- **WHEN** user runs `superpowers status --change <id>` without `--schema`
- **AND** the change has `.superpowers.yaml` with `schema: tdd`
- **THEN** the system uses the `tdd` schema

#### Scenario: Explicit schema overrides metadata

- **WHEN** user runs `superpowers status --change <id> --schema spec-driven`
- **AND** the change has `.superpowers.yaml` with `schema: tdd`
...
- **AND** the change has no `.superpowers.yaml` file
