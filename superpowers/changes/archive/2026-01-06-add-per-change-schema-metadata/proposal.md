...
**Scope: Experimental artifact workflow only** (`superpowers new change`, `superpowers status`, `superpowers instructions`, `superpowers templates`)

- Store schema choice in `.superpowers.yaml` metadata file when creating a change via `superpowers new change`
...
- Add `--schema` option to `superpowers new change` command

**Not affected**: Legacy commands (`superpowers validate`, `superpowers archive`, `superpowers list`, `superpowers show`)
...
- **Backward compatible**: Changes without `.superpowers.yaml` fall back to `spec-driven` default
