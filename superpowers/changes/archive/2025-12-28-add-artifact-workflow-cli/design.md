...
```
superpowers status --change <id>
superpowers next --change <id>
superpowers instructions <artifact> --change <id>
superpowers templates [--schema <name>]
...
```bash
superpowers status --change add-auth   # explicit, works
superpowers status                      # error: missing --change
```
...
```bash
superpowers new change add-feature
```

**Rationale:**
- `superpowers new <name>` is ambiguous (new what?)
- `superpowers new change <name>` is clear and extensible
- Can add `superpowers new spec <name>` later if needed
