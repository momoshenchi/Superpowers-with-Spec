## Why
The current `superpowers init` flow assumes a single assistant selection and stops once an Superpowers structure already exists. That makes onboarding feel rigid: teams cannot configure multiple tools in one pass, they do not learn which files were refreshed, and the success copy always references Claude even when other assistants are involved.

## What Changes
- Allow selecting multiple assistants during `superpowers init`, including refreshing existing configurations in a single run.
...
- Align generated AI-instruction content and specs so CLAUDE.md and AGENTS.md share the same Superpowers guidance.
