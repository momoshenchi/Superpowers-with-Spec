# Update Superpowers Agent Instructions

## Why

The current Superpowers agent instructions need updates to follow best practices for AI assistant instructions (brevity, clarity, removing ambiguity), ensure CLI commands are current with the actual implementation, and properly document the three-stage workflow pattern that agents should follow.
...
- **Complete command documentation** with all current functionality:
  - `superpowers init [path]` - Initialize Superpowers in a project
  - `superpowers list` - List all active changes (default)
  - `superpowers list --specs` - List all specifications
  - `superpowers show [item]` - Display change or spec with auto-detection
  - `superpowers show` - Interactive mode for selection
  - `superpowers diff [change]` - Show spec differences for a change
  - `superpowers validate [item]` - Validate changes or specs
  - `superpowers archive [change]` - Archive completed change after deployment
  - `superpowers update [path]` - Update Superpowers instruction files
...
- **Remove deprecated command references** (noun-first patterns like `superpowers change show`)
- **Add concrete examples** for each command variation
- **Document debugging commands**:
  - `superpowers show [change] --json --deltas-only` for inspecting deltas
  - `superpowers validate [change] --strict` for comprehensive validation
...
- **Delta detection debugging**:
  - Use `superpowers show [change] --json --deltas-only` to inspect parsed deltas
...
- **Spec discovery workflow** - Always check existing specs before creating new ones:
  - Use `superpowers list --specs` to see all current specs
...
- Affected documentation:
  - `superpowers/README.md` - Main Superpowers instructions
  - CLAUDE.md files generated by `superpowers init` command

Note: This is a tooling/infrastructure change that doesn't require spec updates. When archiving, use `superpowers archive update-agent-instructions --skip-specs`.