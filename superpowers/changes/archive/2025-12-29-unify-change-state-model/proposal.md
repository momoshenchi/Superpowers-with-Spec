...
Result: `superpowers new change foo && superpowers view` shows `foo` as "Completed" when it has no content.
...
Result: `superpowers status --change foo` says "not found" even though the directory exists.
...
|---------|--------|-------|
| `superpowers view` | Empty = "Completed" | Empty = "Draft" |
| `superpowers status --change X` | Requires proposal.md | Works on any directory |
| `superpowers validate X` | Requires proposal.md | Unchanged (still requires it) |

## Breaking Changes

### Minimal Breaking Change

1. **`superpowers view` output**: Empty changes move from "Completed" section to new "Draft" section

### Non-Breaking

- Active/Completed semantics unchanged (still task-based)
- `getActiveChangeIds()` unchanged
- `superpowers validate` unchanged
...
|---------|-----------------------------|-----------------------------|----------------|
| `superpowers view` | No (reads dirs directly) | Yes → Draft section | **Yes** |
| `superpowers list` | No (reads dirs directly) | Yes (shows "No tasks") | No |
| `superpowers status/next/instructions` | Yes | Yes | **Yes** |
| `superpowers validate` | Yes | No (can't validate empty) | No |
| `superpowers show` | Yes | No (nothing to show) | No |
| Tab completions | Yes | Future enhancement | No |

## Success Criteria

1. `superpowers new change foo && superpowers view` shows `foo` in "Draft" section
2. `superpowers new change foo && superpowers status --change foo` works
