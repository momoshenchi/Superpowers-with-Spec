...
|--------|---------|---------|
| **Task Progress** | Track implementation work | `superpowers view`, `superpowers list` |
| **Artifact Progress** | Track planning/spec work | `superpowers status`, `superpowers next` |
...
async function validateChangeExists(changeName: string, projectRoot: string): Promise<string> {
  const changePath = path.join(projectRoot, 'superpowers', 'changes', changeName);
  ...
  if (!fs.existsSync(changePath) || !fs.statSync(changePath).isDirectory()) {
    ...
    const entries = await fs.promises.readdir(
      path.join(projectRoot, 'superpowers', 'changes'),
      { withFileTypes: true }
    );
    ...
    if (available.length === 0) {
      throw new Error('No changes found. Create one with: superpowers new change <name>');
    }
...
```bash
# Before
$ superpowers new change foo
$ superpowers status --change foo
