...
The experimental workflow (SP) provides a schema-driven, artifact-by-artifact approach to creating changes with `/sp:new`, `/sp:continue`, `/sp:ff`, `/sp:apply`, and `/sp:sync`. However, there's no corresponding archive command to finalize and archive completed changes. Users must currently fall back to the regular `superpowers archive` command, which doesn't integrate with the SP philosophy of agent-driven spec syncing and schema-aware artifact tracking.
...
- Integrate with existing SP commands for a cohesive workflow
...
- No changes to existing archive command or other SP commands
- Completes the SP command suite for full lifecycle management
