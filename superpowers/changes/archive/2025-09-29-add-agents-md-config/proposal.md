...
## Summary
- Teach `superpowers init` to manage a root-level `AGENTS.md` file using the same marker system as `CLAUDE.md`.
- Allow `superpowers update` to refresh or scaffold that root `AGENTS.md` so AGENTS-compatible tools always receive current instructions.
- Keep the existing `superpowers/AGENTS.md` template as the canonical source while ensuring assistants that read `AGENTS.md` opt-in instructions get the latest guidance automatically.

## Motivation
The README now points teams to AGENTS.md-compatible assistants, but the CLI only manages `CLAUDE.md`. Projects must hand-roll a root `AGENTS.md` file to benefit from the standard, and updates will drift unless maintainers remember to copy content manually. Extending `init` and `update` closes that gap so Superpowers actually delivers on the promise of first-class AGENTS support.

## Proposal
1. Extend the `superpowers init` selection flow with an "AGENTS.md standard" option that creates or refreshes a root `AGENTS.md` file wrapped in Superpowers markers, mirroring the existing CLAUDE integration.
2. When generating the file, pull the managed content from the same template used in `superpowers/AGENTS.md`, ensuring both locations stay in sync.
3. Update `superpowers update` so it always refreshes the root `AGENTS.md` (creating it if missing) alongside `superpowers/AGENTS.md` and any other configured assistants.
...
- **Risk:** Divergence between `superpowers/AGENTS.md` and the root file.
