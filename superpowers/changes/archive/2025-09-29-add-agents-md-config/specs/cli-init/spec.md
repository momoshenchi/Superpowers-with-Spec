...
The command SHALL configure AI coding assistants with Superpowers instructions based on user selection.

#### Scenario: Prompting for AI tool selection

- **WHEN** run
- **THEN** prompt user to select AI tools to configure:
  - Claude Code (✅ Superpowers custom slash commands available)
  - Cursor (✅ Superpowers custom slash commands available)
...
- **THEN** create or update `AGENTS.md` in the project root directory (not inside superpowers/)
...
See @superpowers/AGENTS.md for detailed conventions and guidelines.
<!-- SUPERPOWERS:END -->
```
...
- **THEN** create new file with Superpowers content wrapped in markers using the same template as CLAUDE.md
...
- **THEN** preserve all existing content
- **AND** ensure the Superpowers-managed block at the beginning of the file is refreshed without duplicating markers

#### Scenario: Managing content with markers

- **WHEN** using the marker system
- **THEN** use `<!-- SUPERPOWERS:START -->` to mark the beginning of managed content
- **AND** use `<!-- SUPERPOWERS:END -->` to mark the end of managed content
- **AND** allow Superpowers to update its content without affecting user customizations
