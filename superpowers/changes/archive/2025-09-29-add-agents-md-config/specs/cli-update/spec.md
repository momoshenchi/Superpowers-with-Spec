...
- **THEN** the command SHALL:
  ...
  - Replace `superpowers/AGENTS.md` with the latest template (complete replacement)
...
- **AND** create or update the root-level `AGENTS.md` using the Superpowers markers
- **AND** update only the Superpowers-managed blocks in **existing** AI tool files using markers
- **AND** use the default directory name `superpowers`
...
- **AND** display the message: "Updated Superpowers instructions"
