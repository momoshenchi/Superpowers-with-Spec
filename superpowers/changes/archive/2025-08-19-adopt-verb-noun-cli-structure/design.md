...
2. Add `--specs`/`--changes` to `superpowers list`, with `--changes` as default.
3. Show deprecation warnings for `superpowers change list` and, more generally, for any `superpowers change ...` and `superpowers spec ...` subcommands.
...
## Out of Scope
JSON output parity for `superpowers list` across modes and `show --specs/--changes` discovery are follow-ups.
