...
- Deprecate noun-based top-level commands: `superpowers spec ...` and `superpowers change ...`.
...
- **List**
  - From: `superpowers change list`
  - To: `superpowers list --changes` (default), or `superpowers list --specs`

- **Show**
  - From: `superpowers spec show <spec-id>` / `superpowers change show <change-id>`
  - To: `superpowers show <item-id>` with auto-detect, use `--type spec|change` if ambiguous

- **Validate**
  - From: `superpowers spec validate <spec-id>` / `superpowers change validate <change-id>`
  - To: `superpowers validate <item-id> --type spec|change`, or bulk: `superpowers validate --specs` / `--changes` / `--all`

### Backward Compatibility

- Keep `superpowers spec` and `superpowers change` available with deprecation warnings for one release cycle.
...
- **Affected specs**:
  ...
  - `superpowers-conventions`: Add explicit requirement establishing verb–noun CLI design and deprecation guidance
- **Affected code**:
...
**CLI Design**
- From: Mixed model with nouns (`spec`, `change`) and some top-level verbs; `superpowers list` currently deprecated
- To: Verbs as primary: `superpowers list|show|validate|diff|archive`; nouns scoped via flags or item ids; noun commands deprecated
...
**Listing Behavior**
- From: `superpowers change list` (primary), `superpowers list` (deprecated)
- To: `superpowers list` as primary, defaulting to `--changes`; add `--specs` to list specs
...
- Document new usage in `superpowers/README.md` and CLI help.
