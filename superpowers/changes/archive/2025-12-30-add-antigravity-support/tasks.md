...
- [x] 1.2 Generate `.agent/workflows/superpowers-proposal.md`, `superpowers-apply.md`, and `superpowers-archive.md` with YAML frontmatter restricted to a single `description` field for each stage and wrap the body in Superpowers markers.
...
- [x] 2.1 Detect existing Antigravity workflow files during `superpowers update` and refresh only the managed body, skipping creation when files are missing.
