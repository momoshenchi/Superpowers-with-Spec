...
Superpowers currently has no visibility into how the tool is being used. Without analytics, we cannot:
...
- Store anonymous ID in global config (`~/.config/superpowers/config.json`)
- Respect `DO_NOT_TRACK` and `SUPERPOWERS_TELEMETRY=0` environment variables
