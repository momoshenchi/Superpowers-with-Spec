...
Superpowers needs usage analytics to understand adoption and inform product decisions. PostHog provides a privacy-conscious analytics platform suitable for open source projects.
...
```bash
SUPERPOWERS_TELEMETRY=0    # Disable telemetry
...
- Full `superpowers telemetry` command - Can add later if users request
...
```typescript
{
  ...
  properties: {
    ...
    version: '1.2.3'      // Superpowers version
  }
}
```
...
```
Note: Superpowers collects anonymous usage stats. Opt out: SUPERPOWERS_TELEMETRY=0
```
