...
- [x] 2.1 Create or extend global config module for `~/.config/superpowers/config.json`
...
- [x] 3.1 Implement `isTelemetryEnabled()` checking `SUPERPOWERS_TELEMETRY`, `DO_NOT_TRACK`, and `CI` env vars
...
- [x] 4.3 Display notice text: "Note: Superpowers collects anonymous usage stats. Opt out: SUPERPOWERS_TELEMETRY=0"
...
- [x] 6.1 Test opt-out via `SUPERPOWERS_TELEMETRY=0`
...
- [x] 7.2 Document opt-out methods (`SUPERPOWERS_TELEMETRY=0`, `DO_NOT_TRACK=1`)
