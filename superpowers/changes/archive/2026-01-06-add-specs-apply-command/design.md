...
Currently, delta specs are only applied to main specs when running `superpowers archive`. This bundles two concerns:
...
- Provide `/sp:sync` skill for agents to sync specs on demand
...
**Alternatives considered**:
- Track `specsSynced: true` in `.superpowers.yaml` (rejected: unnecessary complexity)
- Store snapshot of synced deltas (rejected: over-engineering)

### 3. Agent-driven approach (no CLI command)

**Decision**: The `/sp:sync` skill is fully agent-driven - the agent reads delta specs and directly edits main specs.
...
**Rationale**: Backward compatibility. Users who don't use `/sp:sync` get the same experience.
...
→ Running `/sp:sync` again reconciles. Idempotent design handles this.
...
2. Add skill template for `/sp:sync` in `skill-templates.ts`
