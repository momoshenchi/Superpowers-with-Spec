...
- [x] Create `src/core/configurators/slash/github-copilot.ts` implementing `SlashCommandConfigurator` base class
  - Implement `getRelativePath()` to return `.github/prompts/superpowers-{proposal,apply,archive}.prompt.md`
...
- [x] Update `src/core/init.ts` to include GitHub Copilot in the AI tool selection prompt
  - Add GitHub Copilot to the available tools list with detection for existing `.github/prompts/superpowers-*.prompt.md` files
...
- [x] Update `src/core/update.ts` to refresh GitHub Copilot prompts when they exist
  - Call `updateExisting()` for GitHub Copilot configurator when `.github/prompts/` contains Superpowers prompt files
