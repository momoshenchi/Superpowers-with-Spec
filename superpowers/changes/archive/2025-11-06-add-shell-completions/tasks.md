...
- [x] Create `src/core/completions/command-registry.ts` with `COMMAND_REGISTRY` constant defining all Superpowers commands, flags, and metadata
...
- [x] Implement installation to `~/.oh-my-zsh/custom/completions/_superpowers` for Oh My Zsh
- [x] Implement fallback installation to `~/.zsh/completions/_superpowers` with `fpath` updates
...
- [x] Implement dynamic completion helper functions in Zsh generator (`_superpowers_complete_changes`, `_superpowers_complete_specs`, `_superpowers_complete_items`)
...
- [ ] Test dynamic change/spec ID completion in real Superpowers projects
- [ ] Verify completion cache behavior (2-second TTL)
- [ ] Test behavior outside Superpowers projects (should skip dynamic completions)
- [x] Update `superpowers --help` output to include completion command (automatically done via Commander)
