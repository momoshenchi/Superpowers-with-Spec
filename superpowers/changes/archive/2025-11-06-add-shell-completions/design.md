...
```bash
# After installing: superpowers completion install bash
superpowers val<TAB>           # Completes to "superpowers validate"
superpowers validate <TAB><TAB>  # Shows: --all --changes --specs --strict --json [change-ids] [spec-ids]
superpowers show add-<TAB><TAB>  # Shows all changes starting with "add-"
```
...
**Superpowers Integration:**
```zsh
# After installing: superpowers completion install zsh
superpowers val<TAB>    # Shows menu with "validate" and "view" highlighted
superpowers show <TAB>  # Shows menu with all change IDs and spec IDs, categorized
```
...
**Superpowers Integration:**
```fish
# After installing: superpowers completion install fish
superpowers val       # Gray suggestion shows "validate" immediately
superpowers show a    # Real-time suggestions for changes starting with "a"
superpowers <TAB>     # Shows all commands with descriptions in paged menu
```
...
**Superpowers Integration:**
```powershell
# After installing: superpowers completion install powershell
superpowers val<TAB>       # Cycles: validate → view → validate
superpowers show <TAB>     # Cycles through change IDs one by one
superpowers <Ctrl+Space>   # Shows IntelliSense menu with all commands
```
...
- Graceful degradation when outside Superpowers projects
...
│       ├── command-registry.ts    # Single source of truth for Superpowers commands
...
if (isOhMyZshInstalled()) {
  // Install to ~/.oh-my-zsh/custom/completions/_superpowers
  // Automatically loaded by Oh My Zsh
} else {
  // Install to ~/.zsh/completions/_superpowers
...
2. **Not in Superpowers project** - Skip dynamic completions, only offer static commands
...
3. **Manual Testing**
   ...
   - Tab completion behavior in Superpowers projects
