...
4. **Create artifacts in sequence until apply-ready** — after authorized create, read `reference/artifact-loop.md`. If that file is missing, report the stage `blocked` with the missing path; do not skip. Do not inline the per-artifact instruction loop here.
