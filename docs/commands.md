...
- Test Hardening expands `full-qa-test` / 10→10→10 and lands those tests before Git-aware related tests for the non-`test-plan` suite stage when the runner supports Git-aware selection (for example Vitest `--changed` against merge-base). Keep both layers: Git-aware output does not close registered `test-plan.md` rows. If Git-aware selection is unavailable, record `git-aware-unavailable-recorded` (or `git-aware-empty-expected` on instruction-only diffs) and still run registered `test-plan.md` rows; do not run the complete suite as a pass. Record `ran-complete-suite-optional` only when the user asks, CI already requires that complete command, or empty selection is not expected and no focused command can be constructed. Apply then dispatches code review, `/sp:simplify`, and `/sp:design-verify` in parallel when spawn exists, then `/sp:verify` after that pre-Verify wave is clear of P0; missing spawn uses labeled `same-context fallback` and runs those three gates sequentially before Verify. Standalone workflow selection never disables those gates.
- Final-gate retries are bounded and local: only a `P0` forces another CR/DV/Verify round, while `P1`/`P2` are repaired in the current review round without forcing one. The gate outcome `blocked` is a missing prerequisite or external decision, not a severity; it pauses without consuming an attempt. Code review, Verify, and Design verify each stop with `failed` if round four still fails. Simplify has no retry loop. Verify starts only after the pre-Verify wave is clear of P0.
...
- Discovers the canonical non-visual runner and prefers Git-aware related tests for the non-`test-plan` suite stage when supported; if Git-aware selection is unavailable, records `git-aware-unavailable-recorded` and still runs registered `test-plan.md` rows before Manual Coverage
...
- When delegated by apply, labels fresh Verify rounds 1–4; Apply-FQG reuses Hardening Git-aware evidence when implementation and Git baseline are unchanged, and still runs registered `test-plan.md` rows plus deferred `agent-browser` coverage. Standalone `/sp:verify` always runs the canonical non-visual preflight (Git-aware related tests when supported; if unavailable, record `git-aware-unavailable-recorded` and still run registered `test-plan.md` rows). Missing spawn uses labeled `same-context fallback`. Repairable failures retry from Verify, while a `blocked` prerequisite pauses without consuming a round and a fourth failed round is terminal
...
It always runs all four angles. Each of Surface, Boundaries, Model, and Composition reports `P0`, `P1`, or `P2` (highest finding on that angle), `passed` when assessed with no findings, or `not applicable` with evidence. Remaining suggestions are a table with `Sev`, `Angle`, `Location`, `Problem`, and `Suggestion`:
